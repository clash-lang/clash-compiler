  ...
  go opts = traceIf (hasDebugInfo TryTerm name opts) ("Tried: " ++ name ++ " on:\n" ++ before) $ do
    ...
    Monad.when (dbg_invariants opts && hasChanged) $ do
      ...
      let beforeTy = inferCoreTypeOf tcm exprOld
          ...
          filterFVs
            | allowNewFVsFromCtx = Set.filter notInCtx
            | otherwise = id
      ...
      Monad.when
        ( hasDebugInfo AppliedTerm name opts
            && not (normalizeType tcm beforeTy `aeqType` normalizeType tcm afterTy)
        ) $
        traceM
          ( concat
              [ $(curLoc)
    ...
    Monad.when (dbg_invariants opts && not hasChanged && exprNotEqual) $
      error $
        $(curLoc)
          ++ "Expression changed without notice("
          ++ name
          ++ "): before"
          ++ before
          ++ "\nafter:\n"
    ...
    traceIf
      (hasDebugInfo AppliedName name opts && hasChanged)
      (name <> " {" <> show nTrans <> "}")
      $ traceIf
        (hasDebugInfo AppliedTerm name opts && hasChanged)
      ...
      $ traceIf
        (hasDebugInfo TryTerm name opts && not hasChanged)
...
runRewriteSession r s m = do
  (a, s', _) <- runR m r s
  traceIf
    (dbg_countTransformations (opt_debug (envOpts (_clashEnv r))))
    ("Clash: Transformations:\n" ++ Text.unpack (showCounters (s' ^. transformCounters)))
    $ traceIf
      (None < dbg_transformationInfo (opt_debug (envOpts (_clashEnv r))))
    ...
    let e1 = substTm "substInl" subst e
        ...
        subst1 =
          subst
            { substTmEnv =
                mapVarEnv
                  (substTm "substSubst" substE)
                  (substTmEnv subst)
            }
        subst2 = extendIdSubst subst1 x e1
     in if x `elemFreeVars` e1
          then
...
liftAndSubsituteBinders inScope toLift toKeep body = do
  ...
  pure
    ( map (second (substTm "liftToKeep" subst)) toKeep
    ...
    let substE = extendIdSubst (mkSubst inScope) x e2
        subst1 =
          subst
            { substTmEnv =
                mapVarEnv
                  (substTm "liftSubst" substE)
                  (substTmEnv subst)
            }
        subst2 = extendIdSubst subst1 x e2
    if x `elemFreeVars` e2
      then do
...
liftBinding (var@Id{varName = idName}, e) = do
  ...
  aeqExisting <-
    (UniqMap.elems . UniqMap.filter ((`aeqTerm` newBody) . bindingTerm)) <$> Lens.use bindings
  case aeqExisting of
...
removeUnusedBinders (Rec binds) body =
  case eltsVarEnv used of
    [] -> Just body
    qqL
      | not (List.equalLength qqL binds) ->
 ...
 where
  ...
  collectUsed env v =
    if v `elemVarEnv` env
      then
        env
      else case lookupVarEnv v bindsEnv of
        Just (x, e0) ->
