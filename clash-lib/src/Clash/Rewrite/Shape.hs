...
which 'Term' constructors it can fire on; each handler receives the
constructor's fields directly. The handler record /is/ the transformation, so
its entry shapes cannot drift from its matching — the failure mode of
hand-written dispatch tables.
...
bucket, in flat-list order. This eliminates the per-attempt 'applyWith'
overhead for transformations that cannot match the node.

Two contracts make this sound:

1. /Unchanged means untouched./ A handler that does not signal change must
   return the node it was given, unmodified; the next bucket member then runs
   on the same fields without re-matching. This is a pre-existing engine
   invariant ("Expression changed without notice" in
   'Clash.Rewrite.Util.applyDebug').

2. /Change re-guards the rest of the bucket./ After a change the term may
   have any constructor, so the remaining members of the entered bucket run
   through their own full handler records (the "guarded suffix") — exactly
   what the pattern matches of a sequential @>->@ chain did. Members
   registered for other constructors run once the surrounding traversal
   re-dispatches on the new shape.
...
stay plain 'Rewrite' values behind 'Clash.Rewrite.Util.apply'. @topLet@, the
one shape-agnostic per-node transformation, uses 'applyAnyShape'.
-}
...
-- offered such nodes. Use the @on*@ builders rather than this record
-- directly.
...
hasShapedHandler handlers = or
  [ isJust (handleVar handlers)
  , isJust (handleData handlers)
  , isJust (handleLiteral handlers)
  , isJust (handlePrim handlers)
  , isJust (handleLam handlers)
  , isJust (handleTyLam handlers)
  , isJust (handleApp handlers)
  , isJust (handleTyApp handlers)
  , isJust (handleLet handlers)
  , isJust (handleCase handlers)
  , isJust (handleCast handlers)
  , isJust (handleTick handlers)
  ]
...
-- Handler builders. Handlers get the matched node itself — to return on the
-- unchanged path with sharing intact — followed by the constructor's fields.
-- The @on*Node@ builders register a plain 'Rewrite' at a single constructor,
-- for transformations that collect application spines themselves.
...
-- | Run a constructor's bucket in bundle order; after a member signals change
-- the remaining members run as the guarded suffix.
-- See Note [shaped transformations].
runBucket
  :: Instrumentation extra
...
-- | Offer the term to each member through its own handler record: the
-- constructor changed mid-bucket, so every remaining member re-guards on the
-- current term.
runGuardedSuffix
  :: Instrumentation extra
  -> [ShapedTransformation extra]
