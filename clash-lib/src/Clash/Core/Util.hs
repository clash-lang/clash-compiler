 ...
 where
  ...
  go n uniqs0 e = fromMaybe (error "extractElems: failed to project elements") $ do
    ...
    (uniqs2, [elNId, restNId, co, el, rest]) <-
      pure $
        mapAccumR mkUniqSystemId uniqs1 $
          zipEqual
...
extractTElems supply inScope lrCon brCon resTy s maxN tree =
 ...
 where
  ...
  go 0 _ ks uniqs0 e = fromMaybe (error "extractTElems: failed to project elements") $ do
    ...
    (uniqs1, [elNId, co, el]) <-
      pure $
        mapAccumR mkUniqSystemId uniqs0 $
          zipEqual
...
mkRTree lrCon brCon resTy = go
 where
  ...
  lrCoTy = case dataConInstArgTys lrCon [(LitTy (NumTy 0)), resTy] of
    Just (x : _) -> x
    _ -> error "impossible"
  brCoTy n = case dataConInstArgTys
    brCon
    [ (LitTy (NumTy n))
    , resTy
    , (LitTy (NumTy (n - 1)))
...
isSignalType tcm ty = go HashSet.empty ty
 where
  go tcSeen (tyView -> TyConApp tcNm args) = case nameOcc tcNm of
    ...
    _
      | tcNm `HashSet.member` tcSeen -> False -- Do not follow rec types
...
dataConInstArgTysE is0 tcm (MkData{dcArgTys, dcExtTyVars, dcUnivTyVars}) inst_tys = do
 ...
 where
  ...
  go exts0 args0 =
    let eqs = catMaybes (map (typeEq tcm) args0)
     in case solveNonAbsurds tcm exts eqs of
          [] ->
            Just args0
          sols ->
            go exts1 args1
...
dataConInstArgTys (MkData{dcArgTys, dcUnivTyVars, dcExtTyVars}) inst_tys =
  ...
  let tyvars = dcUnivTyVars ++ dcExtTyVars
   in if length tyvars == length inst_tys
        then
...
primUCo =
  Prim
    PrimInfo
      { primName = "GHC.Prim.unsafeCoerce#"
...
data Projections where
  Projections ::
    (forall m. (MonadUnique m) => InScopeSet -> Term -> m [Term]) ->
    Projections
...
shouldSplit0 tcm (TyConApp tcNm tyArgs)
  ...
  , let dcArgVs = map (tyView . coreView tcm) dcArgs =
      if any shouldSplitTy dcArgVs && not (isHidden tcNm tyArgs)
        then
          Just
            ( mkApps (Data dc) . (map Right tyArgs ++) . map Left
            , Projections
                ( \is0 subj ->
                    mapM
                      (mkSelectorCase ($(curLoc) ++ "splitArg") is0 tcm subj 1)
                      [0 .. dcArgsLen - 1]
                )
            , dcArgs
            )
  ...
  , [nil, cons] <- tyConDataCons tc =
      if shouldSplitTy (tyView (coreView tcm argTy))
        then
          Just
            ( mkVec nil cons argTy n
 ...
 where
  ...
  splitTy (TyConApp tcNm0 _) =
    nameOcc tcNm0
      `elem` [ "Clash.Signal.Internal.Clock"
 ...
 where
  ...
  go scrutTy@(tyView -> TyConApp tc args) =
    case tyConDataCons (UniqMap.find tc tcm) of
      ...
      dcs
        | dcI > length dcs -> cantCreate $(curLoc) "DC index exceeds max" scrutTy
        | otherwise -> do
            let dc = indexNote ($(curLoc) ++ "No DC with tag: " ++ show (dcI - 1)) dcs (dcI - 1)
            let fieldTys =
                  fromMaybe
                    (cantCreate $(curLoc) "Cannot instantiate dataCon" scrutTy)
  ...
  cantCreate loc info scrutTy =
    error $
      loc
        ++ "Can't create selector "
        ++ show (caller, dcI, fieldI)
        ++ " for: ("
        ++ showPpr scrut
        ++ " :: "
        ++ showPpr scrutTy
