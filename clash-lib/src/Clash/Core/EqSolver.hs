...
import GHC.Builtin.Names (eqPrimTyConKey, typeNatAddTyFamNameKey, unsafeReflDataConKey)
...
solveEq tcm solveSet (coreView tcm -> left, coreView tcm -> right) =
  case (left, right) of
    ...
    _ ->
      ...
      if any (isTypeFamilyApplication tcm) [left, right]
        then
          []
        else case (tyView left, tyView right) of
