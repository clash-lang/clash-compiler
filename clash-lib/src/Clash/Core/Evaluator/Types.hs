...
import Clash.Core.Term (Alt, PrimInfo (..), Term (..), TickInfo, mkApps)
...
instance ClashPretty StackFrame where
  ...
  clashPretty (PrimApply p tys vs ts) =
    hsep
      [ "PrimApply",
        fromPretty (primName p),
        "::",
        fromPpr (coreTypeOf p),
        "; type args=",
        fromPpr tys,
        "; val args=",
        fromPpr (map valToTerm vs),
        "term args=",
        fromPpr ts
      ]
...
heapInsert GlobalId i x m =
  m
    { mHeapGlobalOverlay = extendVarEnv i x (mHeapGlobalOverlay m),
...
heapDelete GlobalId i m =
  m
    { mHeapGlobalOverlay = delVarEnv (mHeapGlobalOverlay m) i,
