...
checkValidSubst subst@(TvSubst inScope tenv) tys a =
  WARN
    ...
    , "inScope" <+> clashPretty inScope
        <> line
        <> "tenv" <+> clashPretty tenv
        <> line
        <> "tenvFVs" <+> clashPretty (freeVarsOf tenv)
        <> line
        <> "tys" <+> fromPpr tys
    )
    WARN
...
lookupIdSubst doc (Subst inScope tmS _ genv) v
  ...
  | otherwise =
      WARN
        (True, "Subst.lookupIdSubst" <+> doc <+> fromPpr v)
        Var
        v
...
acmpTerm' inScope = go (mkRnEnv inScope)
 where
  ...
  go env (Lam b1 e1) (Lam b2 e2) =
    acmpType' env (varType b1) (varType b2)
      `thenCompare` go (rnTmBndr env b1 b2) e1 e2
  go env (TyLam b1 e1) (TyLam b2 e2) =
    acmpType' env (varType b1) (varType b2)
      `thenCompare` go (rnTyBndr env b1 b2) e1 e2
  ...
  go env (Let (Rec bs1) e1) (Let (Rec bs2) e2) =
    compare (length bs1) (length bs2)
      `thenCompare` foldr thenCmpTm EQ (zipWith (go env') rhs1 rhs2)
      `thenCompare` go env' e1 e2
   where
    (ids1, rhs1) = unzip bs1
    (ids2, rhs2) = unzip bs2
  ...
  go env (Case e1 _ a1) (Case e2 _ a2) =
    compare (length a1) (length a2)
      `thenCompare` go env e1 e2
      `thenCompare` foldr thenCmpTm EQ (zipWith (goAlt env) a1 a2)
  go env (Cast e1 l1 r1) (Cast e2 l2 r2) =
    go env e1 e2
      `thenCompare` acmpType' env l1 l2
      `thenCompare` acmpType' env r1 r2
