...
import Clash.Core.Pretty (fromPpr, ppr)
...
data TvSubst
  = TvSubst
      InScopeSet -- Variable in scope /after/ substitution
  ...
  warnPprTrace
    ...
    $ warnPprTrace
      ...
      ( "inScope"
          ...
          <> line
          <> "needsInScope"
          <+> clashPretty needsInScope
      )
    $ a
  where
    needsInScope =
      UniqMap.foldrWithUnique
        (\k _ s -> delVarSetByKey k s)
        (freeVarsOf tys)
        tenv
...
substTyVarBndr subst@(TvSubst inScope tenv) oldVar =
  ...
  where
    newEnv
      | noChange = delVarEnv tenv oldVar
    ...
    newVar
      | noKindChange = uniqAway inScope oldVar
      | otherwise =
          uniqAway
            inScope
  ...
  where
    ...
    goBind subst0 (Rec xs) =
      let (bndrs, rhss) = unzip xs
          ...
          (is2, rhss') =
            List.mapAccumL
              (\isN -> go subst1 {substInScope = isN})
              (substInScope subst1)
              rhss
...
ordVar v1 v2 =
  compare (varKey v1) (varKey v2)
    `thenCompare` ordName (varName v1) (varName v2)
    `thenCompare` ordType (varType v1) (varType v2)
  ...
  where
    ...
    go (ForAllTy tv1 t1) (ForAllTy tv2 t2) =
      go (varType tv1) (varType tv2)
        `thenCompare` acmpTypeLevels
          (lvl + 1)
          (extendVarEnv tv1 lvl tyL)
          (extendVarEnv tv2 lvl tyR)
          t1
          t2
    go (AppTy s1 t1) (AppTy s2 t2) = go s1 s2 `thenCompare` go t1 t2
    go (LitTy l1) (LitTy l2) = compare l1 l2
...
acmpTermLevels !lvl tmL tmR tyL tyR = go
  where
    ...
    underTmBndr b1 b2 =
      acmpTermLevels
        (lvl + 1)
        (extendVarEnv b1 lvl tmL)
        (extendVarEnv b2 lvl tmR)
        tyL
        tyR
    underTyBndr b1 b2 =
      acmpTermLevels
        ...
        tmL
        tmR
        (extendVarEnv b1 lvl tyL)
        (extendVarEnv b2 lvl tyR)
    ...
    go (Let (Rec bs1) e1) (Let (Rec bs2) e2) =
      ...
      compare (length bs1) (length bs2)
        `thenCompare` let (ids1, rhs1) = unzip bs1
                          (ids2, rhs2) = unzip bs2
                       ...
                       in
                          ...
                          --   let (x :: Word) = x in x
                          --
                          goList goType (map varType ids1) (map varType ids2)
                            `thenCompare` (goList under rhs1 rhs2 `thenCompare` under e1 e2)
    -- Note [Case result types and alpha-equivalence]
    go (Case e1 _ a1) (Case e2 _ a2) =
      compare (length a1) (length a2)
        `thenCompare` (go e1 e2 `thenCompare` goAlts a1 a2)
    go (Cast e1 l1 r1) (Cast e2 l2 r2) =
    ...
    goAlt (DataPat c1 tvs1 ids1, e1) (DataPat c2 tvs2 ids2, e2) =
      ...
      compare c1 c2
        `thenCompare` let (lvlTy, tyL', tyR') = extendLevels lvl tvs1 tvs2 tyL tyR
  ...
  where
    ...
    go salt = \case
      ...
      ForAllTy tv t ->
        aTypeHashLevels
          (lvl + 1)
          (extendVarEnv tv lvl tyEnv)
          (go (hashTag salt 5) (varType tv))
          t
      ...
      (DataPat dc tvs ids, e) ->
        let (lvlTy, tyEnv') = extendLevelsOf lvl tvs tyEnv
         ...
         in aTermHashLevels
              ...
              tmEnv'
              tyEnv'
              (hashWithSalt (hashTag salt 0) dc)
              e
