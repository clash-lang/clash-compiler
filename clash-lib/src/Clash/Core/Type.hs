...
import GHC.Base (isTrue#, ord, (==#))
import GHC.Generics (Generic (..))
import GHC.Integer (smallInteger)
import GHC.Integer.Logarithms (integerLogBase#)
import GHC.TypeLits (ErrorMessage (Text, (:<>:)), type TypeError)
...
reduceTypeFamily tcm (tyView -> TyConApp tc tys)
                ...
                case compare i1 i2 of
                  LT ->
                    Name
                      User
                      "GHC.Types.LT"
                      (fromGhcUnique ordLTDataConKey)
                      wiredInSrcSpan
                  EQ ->
                    Name
                      User
                      "GHC.Types.EQ"
                      (fromGhcUnique ordEQDataConKey)
                      wiredInSrcSpan
                  GT ->
                    Name
                      User
                      "GHC.Types.GT"
                      (fromGhcUnique ordGTDataConKey)
                      wiredInSrcSpan
        _ -> Nothing
  ...
  | Just (FunTyCon{tyConSubst = tcSubst}) <- UniqMap.lookup tc tcm =
      let
        -- See [Note: Eager type families]
        tysR = map (argView tcm) tys
       in
        findFunSubst tcm tcSubst tysR
