...
-}
module Clash.Netlist.BlackBox.Parser (runParse)
where
...
pTagE =
  Result <$ string "~RESULT"
    ...
    <|> SigD
      <$> (string "~SIGD" *> brackets' pSigD)
      <*> (Just <$> (brackets' natural'))
    <|> IW64 <$ string "~IW64"
...
pSigD =
  some
    ( pTagE
        <|> (EscapedSymbol SquareBracketOpen <$ string "[\\")
