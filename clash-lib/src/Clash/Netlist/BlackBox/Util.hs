...
import Clash.Netlist.Util (isVoid, stripAttributes, stripVoid, typeSize)
...
setSym bbCtx l = do
  ...
  where
    ...
    setSym' ::
      Element ->
      StateT
        ( IntMap.IntMap N.IdentifierText, -- ~SYM[number] mapping
          ...
          Just Nothing ->
            error
              ( "In BlackBox "
                  ...
                  ++ " ~SYM["
                  ++ Text.unpack nm
                  ++ "] uses the same name as previously used in ~GENSYM.\n"
          ...
          Just (Just _) ->
            error
              ( "In BlackBox "
                  ...
                  ++ show i
                  ++ "]"
                  ++ " would overwrite an earlier ~SYM["
                  ++ Text.unpack nm
                  ++ "].\n"
                  ++ "You can't mix numbered and named symbols."
              )
        ...
        . map
          ( \case
              ...
              e ->
                error $
                  $(curLoc)
                    ++ "Unexpected element in (GEN)SYM symbol name when processing "
                    ++ "blackbox for "
                    ++ bbnm
                    ++ ": "
                    ++ prettyBlackBoxStr [e]
