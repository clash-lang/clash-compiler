...
import Control.Monad.State (State, StateT (..), gets, lift)
...
setSym bbCtx l = do
 ...
 where
  ...
  setSym' ::
    Element ->
    StateT
      ( IntMap.IntMap N.IdentifierText
...
renderBlackBox libs imps includes bb bbCtx = do
  let nms' =
        zipWith
          (\_ i -> "~INCLUDENAME[" <> Text.pack (show i) <> "]")
          includes
          [(0 :: Int) ..]
  ...
  incs' <-
    mapM
      ( onBlackBox
          (fmap (PP.pretty . ($ 0)) . renderTemplate bbNamedCtx)
  ...
  addIncludes $
    zipWith3
      (\nm' ((_, ext), _) inc -> (Text.unpack nm' <.> Data.Text.unpack ext, inc))
      nms
      includes
      incs'
  addLibraries libs'
...
renderElem b (Period n) = do
  ...
  case stripVoid ty of
    ...
    _ ->
      error $
        $(curLoc) ++ "Period: Expected `KnownDomain` or `KnownConfiguration`, not: " ++ show ty
...
renderElem b (Tag n) = do
  ...
  case stripVoid ty of
    ...
    _ ->
      error $
        $(curLoc) ++ "Tag: Expected `KnownDomain` or `KnownConfiguration`, not: " ++ show ty
renderElem b (IF c t f) = do
 ...
 where
  check ::
    (Backend backend) =>
    Bool -> Int -> HDL -> HdlSyn -> RenderEnums -> Element -> State backend Int
  check xOpt iw hdl syn enums c' = case c' of
    ...
    (ActiveEdge edgeRequested n) -> do
      ...
      case domConf of
        VDomainConfiguration _ _ edgeActual _ _ _ ->
          pure $
...
getDomainConf ::
  (Backend backend, HasCallStack) => HWType -> State backend VDomainConfiguration
getDomainConf = generalGetDomainConf domainConfigurations

generalGetDomainConf ::
  forall m.
  (Monad m, HasCallStack) =>
  -- | a way to get the `DomainMap`
  (m DomainMap) ->
  HWType ->
...
renderTag b (Lit n) =
 ...
 where
  ...
  mkLit (BlackBoxE pNm _ _ _ _ bbCtx _)
    | pNm
        `elem` [ "GHC.Int.I8#"
               , "GHC.Int.I16#"
               , "GHC.Int.I32#"
  ...
  mkLit (BlackBoxE pNm _ _ _ _ bbCtx _)
    | pNm
        `elem` [ "Clash.Sized.Internal.Signed.fromInteger#"
...
renderTag b (Repeat [es] [i]) = do
  ...
  let i'' = case (readEither i' :: Either String Int) of
        Left msg ->
          error $ $(curLoc) ++ "Could not parse " ++ show i' ++ ". read reported: " ++ msg ++ "."
        Right n -> n
...
renderTag b CtxName = case bbCtxName b of
  Just nm -> return (Text.fromStrict nm)
  _
    | Identifier t _ <- fst (bbResult "~CTXNAME" b) ->
      ...
      renderOneLine
        <$> ( nest
                ...
                ( string "~INST"
                    ...
                    <> vcat
                      ( mapM
                          (\(a, b) -> string "~INPUT" <+> string "=>" <+> string a <+> string b <+> string "~")
                          rest
                      )
                )
                <> line
...
prettyElem (ActiveEdge e i) =
  renderOneLine
    <$> (string "~ACTIVEEDGE" <> brackets (string (Text.pack (show e))) <> brackets (int i))
...
prettyElem (SigD es mI) = do
  ...
  renderOneLine
    <$> ( maybe
            (string "~SIGDO" <> brackets (string es'))
