...
import Clash.Annotations.Primitive (
  ...
  PrimitiveWarning (WarnAlways, WarnNonSynthesizable),
  extractPrim,
 )
...
import Clash.Driver.Types (
  ClashOpts (opt_color, opt_primWarn, opt_werror),
 )
...
mkBlackBoxContext bbName declType resIds args@(lefts -> termArgs) = do
  ...
  return
    ( Context bbName (zip ress resTys) imps funs [] lvl nm (listToMaybe ctxName2)
  ...
  ((e', t, l), d) <- case hwTyM of
    Nothing
      ...
      | otherwise ->
          return
            (
              ( error ($(curLoc) ++ "Forced to evaluate untranslatable type: " ++ eTyMsg)
              , Void Nothing
              , False
              )
            , []
  ...
  go =
    \case
      P.BlackBoxHaskell bbName wf _usedArgs multiResult funcName (_fHash, func) -> do
        ...
        case bbFunRes of
          Left err -> do
            ...
            let err' =
                  unwords
                    [ $(curLoc) ++ "Could not create blackbox"
                    , "template using"
                    , show funcName
                    , "for"
                    , show bbName ++ "."
                ...
                case resM of
                  Just (dst', _, _) -> do
                    ...
                    let templDecl1 = case primName pInfo of
                          ...
                          _ -> templDecl0
                    return
                      ( BlackBoxE pNm (libraries p) (imports p) (includes p) bbTempl bbCtx bbEParen
                      , ctxDcls ++ templDecl1
                      )
                  -- Render expression as a Noop when requested
                  Nothing | RenderVoid <- renderVoid p -> do
      ...
      P.Primitive pNm _ _
        ...
        | pNm
            `elem` ["GHC.Prim.dataToTagSmall#", "GHC.Prim.dataToTagLarge#"] -> case args of
...
collectMealy dstNm dst tcm (kd : clk : mealyFun : mealyInit : mealyIn : _) = do
  let (lefts -> args0, res0) = collectBndrs mealyFun
      is0 =
        mkInScopeSet
          ( Lens.foldMapOf freeIds unitVarSet res0
  ...
  case normE of
    ...
    (_, [], [], _, [], binders0, Just result) -> do
      ...
      initAssign <- case exprInit of
        ...
        _ -> case sBinders of
          ((b, _) : _) -> do
            assn <- procAssign Blocking (Id.unsafeFromCoreId b) exprInit
            pure [assn]
      ...
      argAssign <- case iBinders of
        ((i, _) : _) -> do
          assn <- contAssign (Id.unsafeFromCoreId i) exprArg
          pure [assn]
      ...
      (clkExpr, clkDecls) <-
        mkExpr
          False
          Concurrent
          (NetlistId (Id.unsafeMake "__MEALY_CLK__") (inferCoreTypeOf tcm clk))
          clk
...
mkFunInput parentName declType resId e =
          ...
          C.Var fun -> do
            ...
            case lookupVarEnv fun topAnns of
              Just _ ->
                error $
                  $(curLoc)
                    ++ "Cannot make function input for partially applied Synthesize-annotated: "
                    ++ showPpr e
              _ -> do
          ...
          Left (TExpr, _, libs, imps, inc, nm, templ') -> do
            onBlackBox
              ...
              ( \bbName bbHash (TemplateFunction k g _) -> do
                  ...
                  return
                    (
                      ( Left (BBFunction bbName bbHash (TemplateFunction k g f'))
                      , assignTy
                      , []
                      , []
                      , []
 ...
 where
  ...
  goExpr app@(collectArgsTicks -> (C.Var fun, args@(_ : _), ticks)) = do
    ...
    if null tyArgs
      then withTicks ticks $ \tickDecls -> do
        ...
        throw
          ( ClashException
              sp
              ($(curLoc) ++ "Not in normal form: Var-application with Type arguments:\n\n" ++ showPpr app)
              Nothing
          )
  goExpr e' = do
    tcm <- Lens.view tcCache
    ...
    (appExpr, appDecls) <-
      mkExpr False declType (NetlistId (Id.unsafeMake "c$bb_res") eType) e'
  ...
  go _ _ (C.Var v) = do
    let assn =
          Assignment (Id.unsafeMake "~RESULT") assignTy (Identifier (Id.unsafeFromCoreId v) Nothing)
    return (Right ((Id.unsafeMake "", [assn]), assignTy))
  go _ _ (Case scrut ty [alt]) = do
    tcm <- Lens.view tcCache
    let sTy = inferCoreTypeOf tcm scrut
    (projection, decls) <-
      mkProjection declType False (NetlistId (Id.unsafeMake "c$bb_res") sTy) scrut ty alt
  ...
  go _ _ (Case scrut ty (alt : alts@(_ : _))) = do
    ...
    let assn =
          [ NetDecl' Nothing resNm resTy Nothing
          , Assignment (Id.unsafeMake "~RESULT") assignTy (Identifier resNm Nothing)
          ]
  ...
  go _ _ e'@(Case _ _ []) =
    error $
      $(curLoc) ++ "Cannot make function input for case without alternatives: " ++ show e'
  go _ _ e'@(TyLam{}) =
