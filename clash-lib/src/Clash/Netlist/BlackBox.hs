...
mkBlackBoxContext bbName declType resIds args@(lefts -> termArgs) = do
  -- Make context inputs
  let resNms = fmap Id.unsafeFromCoreId resIds
    ...
    go =
      \case
        ...
        P.Primitive pNm _ _
          ...
          | pNm
              `elem` [showt 'GHC.Prim.dataToTagSmall#, showt 'GHC.Prim.dataToTagLarge#] -> case args of
  ...
  case nReprs of
    ...
    Just k -> do
      ...
      if k <= 1
        then
          pure
            ( N.Literal (Just (sIw, iw)) (NumLit 0),
              scrutDecls'
            )
        else do
          tag <- Id.make "c$dtt_tag"
          let alts =
                [ ( Just (NumLit (toInteger i)),
          ...
          pure
            ( N.DataCon sIw (DC (Void Nothing, -1)) [Identifier tag Nothing],
              ...
              _ -> do
                ...
                case lookupVarEnv fun normalized of
                  Just _ -> do
                    ...
                    let ComponentMeta {cmWereVoids} = meta
                    ...
                    let portMap = NamedPortMap (outpAssigns ++ inpAssigns)
