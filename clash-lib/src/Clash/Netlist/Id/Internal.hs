...
addMultiple# ::
  (HasCallStack, Foldable t) => IdentifierSet -> t Identifier -> IdentifierSet
...
makeBasicOr# ::
  (HasCallStack) => IdentifierSet -> Text -> Text -> (IdentifierSet, Identifier)
...
nextN# ::
  (HasCallStack) => Int -> IdentifierSet -> Identifier -> (IdentifierSet, [Identifier])
...
suffix# ::
  (HasCallStack) => IdentifierSet -> Identifier -> Text -> (IdentifierSet, Identifier)
...
parseIdentifier# t =
  let (tsRev, extsRev) = go (List.reverse (Text.splitOn "_" t))
   in (Text.intercalate "_" (List.reverse tsRev), extsRev)
 where
  go :: [Text] -> ([Text], [Word])
