...
expandTopEntity san ihwtys (oId, ohwty) topEntityM
  ...
  let
    argHints = map (maybe "arg" (Id.toText . Id.unsafeFromCoreId san) . fst) ihwtys
    resHint = maybe "result" (Id.toText . Id.unsafeFromCoreId san) oId
...
expandTopEntity _ _ _ topEntityM =
