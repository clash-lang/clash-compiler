...
import Clash.Backend (HWKind (..), HasUsageMap (..), hdlHWTypeKind, hdlKind)
...
unexpectedProjectionErrorMsg dataRepr cI fI =
  "Unexpected projection of zero-width type: "
    ++ show (drType dataRepr)
    ++ ". Tried to make a projection of field "
    ++ show fI
    ++ " of "
    ++ constrNm
...
convertToCustomRepr reprs dRepr@(DataRepr' name' size constrs) hwTy =
  ...
  where
    cs = insertVoids $ case hwTy of
      ...
      _ ->
        error
          ( "Found a custom bit representation annotation "
              ++ show dRepr
              ++ ", "
              ++ "but it was applied to an unsupported HWType: "
              ++ show hwTy
              ++ "."
          )
...
coreTypeToHWType builtInTranslation reprs m ty = do
  ...
  where
    ...
    go ::
      ...
      Type ->
      State
        (Map Type (Either String FilteredHWType))
...
mkADT builtInTranslation reprs m tyString tc args = case tyConDataCons (UniqMap.find tc m) of
  ...
  dcs -> do
    ...
    case (dcs, filteredArgHTyss) of
      _
        | any (hasUnconstrainedExistential m) dcs ->
            throwE $
              $(curLoc)
...
mkUniqueNormalized is0 topMM (args, binds, res) = do
  ...
  case resM of
    Just (oports, owrappers, res1, subst0) -> do
      ...
      let exprs1 = map (substTm "mkUniqueNormalized1" subst3) exprs
...
renameBinder binds (i, collectArgsTicks -> (k, args, ticks)) = withTicks ticks $ \_ -> do
  ...
  where
    ...
    go nm is0 bbArgs bbResultTemplates = do
      ...
      be <- Lens.use backend
      let _sameName i0 i1 = nameOcc (varName i0) == nameOcc (varName i1)
...
evalBlackBox (SomeBackend s) bbCtx bb
  | BBFunction _bbName _bbHash (TemplateFunction _usedArgs _verifFunc func) <- bb =
      let layout = LayoutOptions (AvailablePerLine 120 0.4)
       in toStrict (renderLazy (layoutPretty layout (State.evalState (func bbCtx) s)))
  | BBTemplate bbt <- bb =
...
mkUniqueArguments subst0 (Just (ExpandedTopEntity {..})) args = do
  ...
  return
    ( map isNothing et_inputs,
      concat ports,
      concat decls,
      extendInScopeIdList
        (extendIdSubstList subst0 (map snd subst1))
        (map fst subst1)
    )
  where
    go Nothing _var =
...
idToInPort var = do
  ...
  case portM of
    Just (_, hty) -> do
      when
        (containsBiSignalIn hty && not (isBiSignalIn hty))
...
portProductError loc hwty portProduct =
  error $
    loc
      ++ [I.i|
...
mkVectorChain _ elTy [e] =
  DataCon
    (Vector 1 elTy)
    VecAppend
    [e]
mkVectorChain sz elTy (e : es) =
  DataCon
    (Vector sz elTy)
    VecAppend
    [ e,
...
mkTopUnWrapper topEntity annM dstId args tickDecls = do
  ...
  let topName = Id.toText topIdentifier
      topIdentifier =
        flip
          fromMaybe
          compNameM
  ...
  -- output
  let iResult = inpAssigns : wrappers
...
mkTopInstInput epp@(ExpandedPortProduct pNameHint hwty0 ps) = do
  ...
  let (attrs, hwty1) = stripAttributes hwty0
...
expandErrorMessage (AttrError attrs) =
  [I.i|
  ...
  annotation:
    ...
    #{attrs}
|]
expandErrorMessage (PortProductError pn hwty) =
  [I.i|
...
checkTopEntityPorts typeTrans reprs tcm topId topM =
  let (argTys, resTy) = splitCoreFunForallTy tcm (coreTypeOf topId)
        ...
        Right hwtys ->
          let (argHwtys, resHwty) = (init hwtys, last hwtys)
           in
              ...
              expandTopEntityOrErr (map (Nothing,) argHwtys) (Nothing, resHwty) topM `seq`
                ()
...
expandTopEntity ihwtys (oId, ohwty) topEntityM
  | Synthesize {..} <- fromMaybe (defSyn (error $(curLoc))) topEntityM = do
      ...
      let argHints = map (maybe "arg" (Id.toText . Id.unsafeFromCoreId) . fst) ihwtys
      ...
      output <-
        ...
        if isVoid (stripFiltered ohwty) || isBiSignalOut (stripFiltered ohwty)
          then
            pure Nothing
          else
