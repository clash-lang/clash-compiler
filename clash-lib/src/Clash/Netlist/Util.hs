...
mkADT builtInTranslation reprs m tyString tc args = case tyConDataCons (UniqMap.find tc m) of
  ...
  dcs -> do
    ...
    argHTyss0 <-
      mapM (mapM (ExceptT . coreTypeToHWType builtInTranslation reprs m)) substArgTyss
...
hasUnconstrainedExistential tcm dc =
  let eTVs = dcExtTyVars dc
      ...
      isConstrainedBy eTV (ty1, ty2) =
        let
          -- Free FVs in the LHS and RHS of the constraint that are not the
...
mkUniqueResult subst0 (Just (ExpandedTopEntity{et_output = Just iPort})) res = do
  ...
  when
    ...
    ( throw
        ( ClashException
            sp
            ($(curLoc) ++ "BiSignalIn cannot be part of a function's result. Use 'readFromBiSignal'.")
            Nothing
        )
    )
      ...
      when
        ...
        ( throw
            ( ClashException
                sp
                ( $(curLoc)
                    ++ "BiSignalIn currently cannot be part of a composite type when it's a function's argument"
                )
                Nothing
            )
        )
