...
import Clash.Core.Pretty (PrettyOptions (..), ppr, showPpr, showPpr')
    ...
    Just (Binding nm' sp inl pr tm r) -> do
      ...
      if resTyRep
        then do
          ...
          traceIf
            (bindingRecursive tmNorm)
            ( concat
                [ $(curLoc),
                  "Expr belonging to bndr: ",
                  nmS,
                  " (:: ",
...
cleanupGraph topEntity norm
  | Just ct <- mkCallTree [] norm topEntity =
      do
        cache <- liftIO (IORef.newIORef UniqMap.empty)
...
stripArgs allIds [] args =
  if any mentionsId args
    then Nothing
    else Just args
  where
    mentionsId t =
      not $
        null
          ( either (Lens.toListOf freeLocalIds) (const []) t
              `intersect` allIds
          )
      ...
      case norm of
        Right (ids, [(bId, bExpr)], _) -> do
          ...
          case stripArgs ids (reverse ids) (reverse args) of
            Just remainder
              | bId `notElemFreeVars` bExpr ->
                  return (Right ((nm, mkApps (mkTicks fun ticks) (reverse remainder)), []))
            _ -> return (Right ((nm, e), []))
        _ -> return (Right ((nm, e), []))
flattenNode b@(CBranch (_, (Binding _ _ spec _ _ _)) _)
  | isNoInline spec =
      return (Left b)
    ...
    flatten =
      ...
      repeatR
        ...
        !-> bottomupR (apply "deadCode" deadCode)
          >-> topdownSucR (apply "topLet" topLet) -- See #3407
          >->
          ...
          >-> bottomupR (apply "flattenLet" flattenLet) -- https://github.com/clash-lang/clash-compiler/issues/3159 / #3204
          >-> bottomupR (apply "bindConstantVar" bindConstantVar) -- https://github.com/clash-lang/clash-compiler/issues/3185
          >-> topdownSucR (apply "topLet" topLet) -- https://github.com/clash-lang/clash-compiler/issues/3041
