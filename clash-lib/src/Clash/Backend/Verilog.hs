...
module_ c =
 ...
 where
  ...
  outPorts = do
    ...
    sequence
      [sigPort "output" (useOf id_ u) id_ hwType iEM | (u, (id_, hwType), iEM) <- outputs c]
...
inst_ (CondAssignment id_ _ scrut scrutTy@(CustomProduct{}) es) =
  inst_' (Id.toText id_) scrut scrutTy es
inst_ (CondAssignment id_ _ scrut scrutTy es) =
  fmap Just $
    "always @(*) begin"
      <> line
      <> indent
        ...
        ( "case"
            <> parens (expr_ True scrut)
            <> line
            <> (indent 2 $ vcat $ punctuate semi (conds (Id.toText id_) es))
            <> semi
            <> line
            <> "endcase"
        )
 ...
 where
  ...
  conds _ [] = return []
  conds i [(_, e)] =
    ("default" <+> colon <+> stringS i <+> equals <+> expr_ False e) <:> return []
  conds i ((Nothing, e) : _) = ("default" <+> colon <+> stringS i <+> equals <+> expr_ False e) <:> return []
  conds i ((Just c, e) : es') =
    ( exprLitV (Just (scrutTy, conSize scrutTy)) c
        <+> colon
        <+> stringS i
        <+> equals
        <+> expr_ False e
...
buildSplitRange offset eP (s, e) =
  let d = s - e
   in (eP + d + 1, (s + offset, e + offset, Provenance (eP + d) eP))
...
continueWithRange ses hty r = case r of
  Contiguous _ offset -> case ses of
    ...
    ses1 ->
      let ses2 = snd (mapAccumR (buildSplitRange offset) 0 ses1)
       in (Split ses2, hty)
...
customReprDataCon dataRepr constrRepr [] =
  let origins = bitOrigins dataRepr constrRepr :: [BitOrigin]
   in case origins of
        [Lit (bitsToBits -> ns)] ->
...
expr_ _ ( Identifier
            d_
            (Just (Indexed (CustomProduct _id dataRepr _size _maybeFieldNames tys, dcI, fI)))
          )
  | DataRepr' _typ _size [cRepr] <- dataRepr
  , ConstrRepr' _cName _pos _mask _val anns <- cRepr =
      let ranges = map range' (bitRanges (anns !! fI))
       in case fieldTy of
            Void{} -> error (unexpectedProjectionErrorMsg dataRepr dcI fI)
            _ -> braces $ hcat $ punctuate ", " $ sequence ranges
 where
...
expr_ _ (Identifier id_ (Just m)) = case modifier (Contiguous 0 0) m of
  ...
  Just (Contiguous start end, resTy) -> case resTy of
    ...
    _ -> slice start end
  Just (Split rs, resTy) ->
    let rs1 = listBraces (mapM (\(start, end, _) -> slice start end) rs)
     in case resTy of
...
expr_ _ (DataCon (CustomSP _name dataRepr _size constrs) (DC (_, constrNr)) es) =
  let (cRepr, _, argTys) = constrs !! constrNr
   in customReprDataCon dataRepr cRepr (zip argTys es)
...
exprLit ::
  (Backend s) =>
  Lens' s (Maybe (Maybe Int)) -> Maybe (HWType, Size) -> Literal -> Ap (State s) Doc
