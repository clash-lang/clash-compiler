...
genVerilog _ sp c = do
  ...
  where
    ...
    verilog = "// Automatically generated SystemVerilog-2005" <> line <>
              module_ c
...
module_ c = do
  ...
  where
    ports = sequence
          $ [ encodingNote hwty <> string i | (i,hwty) <- inputs c ] ++
            [ encodingNote hwty <> string i | (_,(i,hwty)) <- outputs c]

    inputPorts = case inputs c of
                   [] -> emptyDoc
                   p  -> vcat (punctuate semi (sequence [ "input" <+> sigDecl (string i) ty | (i,ty) <- p ])) <> semi

    outputPorts = case outputs c of
                   [] -> emptyDoc
                   p  -> vcat (punctuate semi (sequence [ "output" <+> sigDecl (string i) ty | (_,(i,ty)) <- p ])) <> semi
