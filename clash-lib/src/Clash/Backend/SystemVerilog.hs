...
data SystemVerilogState
  ...
  , _genDepth :: Int
  -- ^ Depth of current generative block
  , _modNm :: ModName
  , _topNm :: Identifier
...
instance Backend SystemVerilogState where
  initBackend opts =
    SystemVerilogState
      ...
      , _idSeen =
          Id.emptyIdentifierSet (opt_escapedIds opts) (opt_lowerCaseBasicIds opts) SystemVerilog
      , _oports = []
      , _srcSpan = noSrcSpan
      , _includes = []
      , _imports = []
...
topSortHWTys hwtys = sorted
 where
  ...
  edge t@(Vector _ elTy) =
    maybe
      []
      ((: []) . (HashMap.lookupDefault (error $ $(curLoc) ++ "Vector") t nodesI,))
      (HashMap.lookup elTy nodesI)
  edge t@(RTree _ elTy) =
    maybe
      []
      ((: []) . (HashMap.lookupDefault (error $ $(curLoc) ++ "RTree") t nodesI,))
      (HashMap.lookup elTy nodesI)
  edge t@(Product _ _ tys) =
    let ti = HashMap.lookupDefault (error $ $(curLoc) ++ "Product") t nodesI
     in mapMaybe (\ty -> liftM (ti,) (HashMap.lookup ty nodesI)) tys
  edge t@(SP _ ctys) =
    let ti = HashMap.lookupDefault (error $ $(curLoc) ++ "SP") t nodesI
          ...
          Vivado -> case splitVecTy ty of
            ...
            _ ->
              "typedef"
                ...
                <+> brackets (int (typeSize elTy - 1) <> colon <> int 0)
                <+> tyName ty
                <+> brackets (int 0 <> colon <> int (2 ^ n - 1))
                <> semi
          _ -> case splitVecTy ty of
            Just (Right n' : ns, elTy') ->
              -- n' == 2^n
      ...
      Just
        A.<$> "function"
          ...
          <> indent
            2
            ( "for"
                <+> parens ("int n = 0" <> semi <+> "n <" <+> int n <> semi <+> "n=n+1")
                <> line
          ...
          <> if n > 1
            then
              "function"
                ...
                <+> tName
                <> "_cons"
                <> parens (sigDecl "x" elTy <> comma <> vecSigDecl "xs")
                <> semi
                <> line
                <> indent
                  2
                  ( tName
                      ...
                      <> brackets (int 0)
                      <+> "="
                      <+> (toSLV elTy (Identifier (Id.unsafeMake "x") Nothing))
                      <> semi
                      <> line
                      <> tName
                      <> "_cons"
                      <> brackets (int 1 <> colon <> int (n - 1))
                      <+> "="
                      <+> "xs"
                      <> semi
                  )
      ...
      Just
        A.<$> "function"
          ...
          <> indent
            2
            ( "for"
                <+> parens ("int n = 0" <> semi <+> "n <" <+> int (2 ^ n) <> semi <+> "n=n+1")
                <> line
                <> indent 2 (tName <> "_to_lv" <> brackets "n" <+> "=" <+> "i[n]" <> semi)
            )
          <> line
          ...
          <> ( if n > 0
                 then
                   "function"
                     ...
                     <+> tName
                     <> "_br"
                     <> parens (treeSigDecl "l" <> comma <> treeSigDecl "r")
                     <> semi
                     <> line
                     <> indent
                       2
                       ( tName
                           <> "_br"
                           <> brackets (int 0 <> colon <> int (2 ^ (n - 1) - 1))
                           <+> "="
                           <+> "l"
                           <> semi
                           <> line
                           <> tName
                           <> "_br"
                           <> brackets (int (2 ^ (n - 1)) <> colon <> int (2 ^ n - 1))
                           <+> "="
                           <+> "r"
                           <> semi
                       )
...
module_ c =
 ...
 where
  ...
  modBody =
    indent 2 (decls (declarations c)) <> line <> line <> indent 2 (insts (declarations c))
  modEnding = "endmodule"
...
patLitCustom (CustomSP _name _dataRepr size reprs) (NumLit (fromIntegral -> i)) =
  let (cRepr, _id, _tys) = reprs !! i
   in patLitCustom' size cRepr
patLitCustom x y =
  error $
    $(curLoc)
      ++ unwords
...
inst_' ::
  Identifier -> Expr -> HWType -> [(Maybe Literal, Expr)] -> SystemVerilogM (Maybe Doc)
      ...
      verilogType ty
        ...
        <> indent
          ...
          ( "if"
              ...
              <> line
              <> (indent 2 $ pretty regId <+> equals <+> expr_ False t <> semi)
              <> line
              <> "else"
              <> line
              <> (indent 2 $ pretty regId <+> equals <+> expr_ False f <> semi)
          )
        <> line
        <> "end"
        <> line
      ...
      "always_comb begin"
        ...
        <> indent
          ...
          ( "if"
              <> parens (expr_ True scrut)
              <> line
              <> (indent 2 $ pretty id_ <+> equals <+> expr_ False t <> semi)
              <> line
              <> "else"
              <> line
              <> (indent 2 $ pretty id_ <+> equals <+> expr_ False f <> semi)
          )
        <> line
        <> "end"
 where
...
inst_ (CondAssignment id_ ty scrut scrutTy es) = fmap Just $ do
      ...
      verilogType ty
        ...
        <> indent
          ...
          ( "case"
              <> parens (expr_ True scrut)
              <> line
              <> (indent 2 $ vcat $ punctuate semi (conds regId es))
              <> semi
              <> line
              <> "endcase"
          )
      ...
      "always_comb begin"
        ...
        <> indent
          ...
          ( "case"
              <> parens (expr_ True scrut)
              <> line
              <> (indent 2 $ vcat $ punctuate semi (conds id_ es))
              <> semi
              <> line
              <> "endcase"
          )
 ...
 where
  ...
  conds i ((Just c, e) : es') =
    ( exprLitSV (Just (scrutTy, conSize scrutTy)) c
        <+> colon
        <+> pretty i
        <+> equals
        <+> expr_ False e
      ...
      <:> conds i es'
inst_ (InstDecl _ _ attrs nm lbl ps pms0) =
  fmap Just $
    attrs' <> nest 2 (pretty nm <> params <> pretty lbl <> line <> pms2 <> semi)
 where
  pms2 = case pms0 of
    NamedPortMap pms1 ->
      let pm i e = dot <> expr_ False i <+> parens (expr_ False e)
       in tupled $ sequence [pm i e | (i, _, _, e) <- pms1]
    IndexedPortMap pms1 ->
  ...
  params
    ...
    | otherwise =
        line
          <> "#"
          <> tupled (sequence [dot <> expr_ False i <+> parens (expr_ False e) | (i, _, e) <- ps])
          <> line
  attrs'
    | null attrs = emptyDoc
     ...
     in let expr' = argExprs !! n
         in if
              ...
              | otherwise ->
                  ...
                  let rotated = parens expr' <+> ">>" <+> int end
                   in int fsize <> squote <> parens rotated

seq_ :: Seq -> SystemVerilogM Doc
seq_ (AlwaysClocked edge clk ds) =
  "always @"
    <> parens
      ( case edge of Rising -> "posedge"; _ -> "negedge"
          <+> expr_ False clk
      )
    <+> "begin"
    <> line
...
expr_ _ ( Identifier
            id_
            (Just (Indexed (CustomProduct _id dataRepr _size _maybeFieldNames args, dcI, fI)))
          ) =
  case fieldTy of
    Void{} ->
      error (unexpectedProjectionErrorMsg dataRepr dcI fI)
...
expr_ _ (Identifier id_ (Just (Indexed (ty@(Product _ _ tys), _, fI)))) = do
  id' <-
    fmap (Text.toStrict . renderOneLine) (pretty id_ <> dot <> tyName ty <> "_sel" <> int fI)
...
expr_ _ (DataCon ty@(CustomSum _ _ _ tys) (DC (_, i)) []) =
  let (ConstrRepr' _ _ _ value _) = fst $ tys !! i
   in int (typeSize ty) <> squote <> "d" <> int (fromIntegral value)
...
expr_ _ (BlackBoxE pNm _ _ _ _ bbCtx _)
  ...
  , [Literal _ (NumLit n), Literal _ (NumLit m), Literal _ (NumLit i)] <-
      extractLiterals bbCtx =
...
expr_ _ (DataTag Bool (Right id_)) = do
  iw <- Ap $ use intWidth
  "$unsigned"
    <> parens (listBraces (sequence [braces (int (iw - 1) <+> braces "1'b0"), pretty id_]))
...
rtreeChain (DataCon (RTree _ _) _ [e1, e2]) =
  A.liftA2
    (++)
    (rtreeChain e1)
    (rtreeChain e2)
...
fromSLV t@(Vector _ _) id_ start end =
  verilogTypeMark t
    <> "_from_lv"
    <> parens (pretty id_ <> brackets (int start <> colon <> int end))
fromSLV t@(RTree _ _) id_ start end =
  verilogTypeMark t
    <> "_from_lv"
    <> parens (pretty id_ <> brackets (int start <> colon <> int end))
...
modifier offset mods (Sliced (BitVector _, start, end)) =
  let m = Right (NRange (start + offset) (end + offset))
   in case mods of
