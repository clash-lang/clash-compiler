...
type ResolvedPrimitive =
  Primitive Text ((TemplateFormat, BlackBoxFunctionName), Maybe Text) () (Maybe Text)
...
parseBBFN bbfn =
  case splitOn "." bbfn of
    ...
    nms ->
      let (mods, func) = (init nms, last nms)
       in let errs = lefts $ checkFunc func : map checkMod mods
           in case errs of
