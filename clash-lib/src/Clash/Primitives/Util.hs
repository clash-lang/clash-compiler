...
resolvePrimitive' metaPath BlackBox {template = t, includes = i, resultNames = r, resultInits = ri, ..} = do
  ...
  bb <-
    BlackBox name workInfo renderVoid multiResult kind () outputUsage libraries imports functionPlurality
...
resolvePrimitive fileName = do
  entries <-
    (decoder fileName :: LZ.ByteString -> [UnresolvedPrimitiveEntry])
  ...
  primitiveFiles <- fmap concat $ forM filePaths $ \filePath -> do
    ...
    if fpExists
      then do
        ...
        let jsonPrims = filter (".primitives" `isSuffixOf`) contents
