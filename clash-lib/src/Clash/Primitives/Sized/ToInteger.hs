...
import Clash.Annotations.Primitive (HDL (VHDL, Verilog))
import Clash.Core.Type (LitTy (NumTy), Type (LitTy))
  ...
  case args of
    (Right (LitTy (NumTy i)) : _) -> do
      ...
      when (fromInteger i1 > iw) $ do
        ...
        warnAboutM WarnIntegerNarrowing sp (showSDocUnsafe msg)
    _ -> return ()
  return ((meta,) <$> bb)
  where
    ...
    bb =
      BBTemplate <$> case runParse (pack bbText) of
        Success t -> Right t
    ...
    warnMsg i iw =
      tyName
        Outputable.<> text ".toInteger: Integer width,"
        <+> int iw
        Outputable.<> text ", is smaller than"
        <+> tyName
        <+> text "width,"
        <+> integer i
        Outputable.<> text ". Dropping MSBs."
