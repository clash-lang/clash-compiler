...
-- 'TemplateFunction'.
data InterpFunctionType
  = InterpBlackBoxFunction
  | InterpTemplateFunction
...
data InterpFunctionRequest = InterpFunctionRequest
  { ifrModNames :: [String]
    -- ^ Module the function lives in, one entry per module name component
  , ifrFuncName :: String
    -- ^ Function name
  , ifrType :: InterpFunctionType
    -- ^ Type to interpret the function at
  , ifrSource :: Maybe Text
    -- ^ Inline Haskell source of the module, if the primitive provided one
  } deriving (Eq, Show, Generic, Hashable)

-- | Module the function of an 'InterpFunctionRequest' lives in, as a
-- qualified module name
ifrQualMod :: InterpFunctionRequest -> Hint.ModuleName
ifrQualMod = intercalate "." . ifrModNames

-- | The result of interpreting an 'InterpFunctionRequest'
data InterpFunctionResult
  = InterpBBF BlackBoxFunction
  | InterpTF TemplateFunction

type InterpResults =
  HashMap InterpFunctionRequest
          (Either (NonEmpty Hint.InterpreterError) InterpFunctionResult)
...
-- 'knownTemplateFunctions' need no interpretation and yield no request. NB:
-- keep this function in sync with the interpreter results consumed by
-- 'compilePrimitiveWith'.
neededInterpRequests :: ResolvedPrimitive -> [InterpFunctionRequest]
neededInterpRequests (BlackBoxHaskell _ _ _ _ bbGenName source)
  | HashMap.member fullName knownBlackBoxFunctions = []
  | otherwise =
      [InterpFunctionRequest modNames funcName InterpBlackBoxFunction source]
 where
...
neededInterpRequests (BlackBox _ _ _ _ _ _ _ _ _ _ incs rM riM templ) =
 ...
 where
  ...
  go ((THaskell, BlackBoxFunctionName modNames funcName), source@(Just _)) =
    [InterpFunctionRequest modNames funcName InterpTemplateFunction source]
  go ((THaskell, BlackBoxFunctionName modNames funcName), Nothing)
    | HashMap.member fullName knownTemplateFunctions = []
    | otherwise =
        [InterpFunctionRequest modNames funcName InterpTemplateFunction Nothing]
   where
    fullName = intercalate "." modNames ++ "." ++ funcName
...
-- faster than a session (or two) per function. For each request this action
-- tries two things, mirroring the historical per-function behavior:
--
...
interpretFunctions idirs pkgDbs topDir reqs = do
  sessionRes <- Hint.unsafeRunInterpreterWithArgsLibdir interpreterArgs topDir $ do
    ...
    globalResults <- forM reqs $ \req -> do
      Hint.liftIO $ Monad.when debugIsOn $
        putStr "Hint: Interpreting " >> putStrLn (ifrQualMod req ++ "." ++ ifrFuncName req)
 ...
 where
  ...
  tryInterp
    :: Hint.InterpreterT IO InterpFunctionResult
    -> Hint.InterpreterT IO (Either Hint.InterpreterError InterpFunctionResult)
  tryInterp = try

  interpret req = case ifrType req of
    InterpBlackBoxFunction ->
      InterpBBF <$> Hint.unsafeInterpret (ifrFuncName req) "BlackBoxFunction"
    InterpTemplateFunction ->
      InterpTF <$> Hint.unsafeInterpret (ifrFuncName req) "TemplateFunction"

  interpretGlobal req = do
    Hint.setImports
      ["Clash.Netlist.Types", "Clash.Netlist.BlackBox.Types", ifrQualMod req]
    interpret req

  interpretLocal defaultPath req = withSourceDir $ \extraDirs -> do
    Hint.set [Hint.searchPath Hint.:= (extraDirs ++ idirs ++ defaultPath)]
    Hint.loadModules [ifrQualMod req]
    Hint.setImports
      ["Clash.Netlist.BlackBox.Types", "Clash.Netlist.Types", ifrQualMod req]
    interpret req
   where
...
compilePrimitives idirs pkgDbs topDir primMapR = do
  let reqs = HashSet.toList $ HashSet.fromList $
        concatMap neededInterpRequests
...
lookupInterpResult
  :: InterpResults
  -> InterpFunctionRequest
  -> IO (Either (NonEmpty Hint.InterpreterError) InterpFunctionResult)
...
compilePrimitiveWith
  :: (InterpFunctionRequest -> IO (Either (NonEmpty Hint.InterpreterError) InterpFunctionResult))
...
compilePrimitiveWith lookupInterp (BlackBoxHaskell bbName wf usedArgs multiRes bbGenName source) = do
  bbFunc <-
    case HashMap.lookup fullName knownBlackBoxFunctions of
      Just f -> pure f
      Nothing -> do
        r <- lookupInterp (InterpFunctionRequest modNames funcName InterpBlackBoxFunction source)
 ...
 where
    ...
    expectBBF :: InterpFunctionResult -> IO BlackBoxFunction
    expectBBF (InterpBBF f) = pure f
...
compilePrimitiveWith lookupInterp
 ...
 where
  ...
  interpretTF :: InterpFunctionRequest -> IO TemplateFunction
  interpretTF req = do
    r <- lookupInterp req
    ...
    case res of
      ...
      _ -> error ($(curLoc) ++ "Internal error: expected a TemplateFunction for "
                         ++ ifrQualMod req ++ "." ++ ifrFuncName req)
  ...
  parseBB ((THaskell,bbGenName),source@(Just source')) = do
    ...
    BBFunction (Data.Text.unpack pNm) hsh <$>
      interpretTF (InterpFunctionRequest modNames funcName InterpTemplateFunction source)
  parseBB ((THaskell,bbGenName),Nothing) = do
    ...
    tf <-
      case HashMap.lookup fullName knownTemplateFunctions of
        Just f -> pure f
        Nothing ->
          interpretTF (InterpFunctionRequest modNames funcName InterpTemplateFunction Nothing)
