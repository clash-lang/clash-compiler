...
import Clash.Netlist.Types (CommentOrDirective (..), Declaration (..), Expr, Seq (..))
...
pprProperty (Property prop0) =
  let prop1 = fromMaybe "__autogen__" . fst <$> prop0
   in pprPslProperty VHDL "prop" "clk" Rising prop1
...
pprYosysSvaProperty ::
  ...
  Property' Text ->
  Declaration
pprYosysSvaProperty propName clk edge assertion =
  ConditionalDecl
    "FORMAL"
