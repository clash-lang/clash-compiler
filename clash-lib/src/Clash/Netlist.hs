...
import Clash.Driver.Types (Binding (..), BindingMap, ClashEnv (..), ClashOpts (..))
...
mkNetDecl (id_, tm) | (_, _, ticks) <- collectArgsTicks tm = preserveVarEnv $ withTicks ticks $ \_ -> do
  ...
  where
    multiDecls srcNote pInfo args0 = do
      ...
      let resInits1 = map Just resInits0 <> repeat Nothing
      ...
      | let ty = coreTypeOf (topId topEntity),
        ...
        length fArgTys1 == length args ->
          do
            ...
            let filteredTypeExprs = filter (not . isVoid . snd) (zip argExprs argHWTys)
    ...
    (True, Nothing) -> do
      ...
      case lookupVarEnv fun normalized of
        ...
        Just (Binding {bindingTerm}) -> do
          ...
          -- Filter void arguments, but make sure to render their declarations:
          let argTypeExprs = zip argHWTys (zip argExprs argTys)
