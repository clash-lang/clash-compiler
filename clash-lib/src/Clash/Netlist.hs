  ...
  case compExprM of
    Nothing -> do
      ...
      throw
        ( ClashException
            sp
            ($(curLoc) ++ "No normalized expression found for: " ++ show compName)
            Nothing
        )
    Just b -> do
...
mkNetDecl (id_, tm) | (_, _, ticks) <- collectArgsTicks tm = preserveVarEnv $ withTicks ticks $ \_ -> do
  ...
  if
    | not (shouldRenderDecl hwTy tm) -> return []
...
mkSelection declType bndr scrut altTy alts0 tickDecls = do
  ...
  case iteAlts scrutHTy (NE.toList alts0) of
      ...
      , Concurrent <- declType ->
          do
            (scrutExpr, scrutDecls) <- case scrutHTy of
              SP{} ->
                first (mkScrutExpr sp scrutHTy (fst (NE.last alts0)))
            ...
            -- TODO: Fix ^
            if
              | isVoid altHTy && isVoid scrutHTy ->
                  return $! scrutDecls ++ altTDecls ++ altFDecls
              | isVoid altHTy ->
                  return $! altTDecls ++ altFDecls
              | otherwise ->
                  do
                    dstAssign <- contAssign dstId (IfThenElse scrutExpr altTExpr altFExpr)
    ...
    _ -> do
      ...
      (scrutExpr, scrutDecls) <-
        first (mkScrutExpr sp scrutHTy (fst (NE.head alts1)))
          <$> mkExpr True declType (NetlistId scrutId scrutTy) scrut
      ...
      case declType of
        ...
        Concurrent ->
          if
            ...
            | otherwise ->
                do
                  assign <- condAssign dstId altHTy scrutExpr scrutHTy exprs
 ...
 where
  ...
  mkScrutExpr sp scrutHTy pat scrutE = case pat of
    DataPat dc _ _ ->
      let modifier = Just (DC (scrutHTy, dcTag dc - 1))
       in case scrutE of
  ...
  altAssign i (m, expr) ds = do
    ...
    assn <- case expr of
      Noop -> pure []
      _ -> do
        assn <- procAssign Blocking i expr
        pure [assn]
...
patPos reprs pat@(DataPat dataCon _ _) =
  ...
  let name = nameOcc $ dcName dataCon
   in case getConstrRepr name reprs of
        Nothing ->
          -- TODO: err
    ...
    (True, Just topEntity)
      ...
      , length fArgTys1 == length args ->
          do
            ...
            (argExprs, concat -> argDecls) <-
              unzip
                <$> mapM
                  (\(e, t) -> mkExpr False declType (NetlistId dstId t) e)
                  (zip args fArgTys1)
      ...
      case lookupVarEnv fun normalized of
        ...
        Just (Binding{bindingTerm}) -> do
          ...
          if length filteredTypeExprs == length compInps
            then do
              (argExprs', argDecls') <-
                (second concat . unzip) <$> mapM (toSimpleVar declType dstId) filteredTypeExprs
   ...
   in withTicks ticks $ \tickDecls -> do
        ...
        let invalid kind =
              throw
                ( ClashException
                    sp
                    ($(curLoc) ++ "Not in normal form: " ++ kind ++ "\n\n" ++ showPpr app)
                    Nothing
                )
        case appF of
          ...
          Case scrut tyA (alt : alts) -> do
            ...
            decls <-
              mkSelection
                declType
                (NetlistId argNm (netlistTypes1 bndr))
                scrut
                tyA
                (alt :| alts)
                tickDecls
    ...
    _ -> case dstHType of
      ...
      Product _ _ dcArgs ->
        case compare (length dcArgs) (length argExprsFiltered) of
          ...
          GT ->
            error $
              unlines
                [ $(curLoc) ++ "Under-applied constructor:" ++ StrictText.unpack dcNm
      ...
      Bool ->
        let dc' = case dcTag dc of
              ...
              tg ->
                error $ $(curLoc) ++ "unknown bool literal: " ++ showPpr dc ++ "(tag: " ++ show tg ++ ")"
         in return dc'
      ...
      String ->
        let dc' = case dcTag dc of
              ...
              _ ->
                error $
                  $(curLoc)
                    ++ "mkDcApplication undefined for: "
                    ++ show (dstHType, dc, dcTag dc, args, argHWTys)
         in return dc'
      Void{} -> return Noop
      Signed _
      ...
      assns <-
        mapMaybeM
          ( \case
              (_, Noop) -> pure Nothing
              (dstId, e) ->
                let nm = netlistId1 id Id.unsafeFromCoreId dstId
                 in case e of
                      Identifier nm0 Nothing
                        | nm == nm0 -> pure Nothing
                      _ ->
                        Just <$> case declType of
