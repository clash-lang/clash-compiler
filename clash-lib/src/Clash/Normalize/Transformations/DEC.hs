...
disjointExpressionConsolidation ctx@(TransformContext isCtx _) e@(Case _scrut _ty _alts@(_ : _ : _)) = do
 ...
 where
  ...
  substLifted isN substitution lifted = do
    ...
    lifted1 <-
      Monad.zipWithM
        (\s (eL, seen) -> collectGlobals isN s seen eL)
        subsMatrix
        lifted
...
collectGlobalsArgs ::
  InScopeSet ->
  {- | Substitution of (applications of) a global
    ...
    List.mapAccumRM
      ( \lbs (c, pos) -> do
          ...
          case (cL, areShared tcm inScope (fmap (first stripTicks) cL)) of
            ...
            (Right _ : _, False) ->
              error
                ( "mkDisjointGroup: non-equal type arguments: "
 ...
 where
  noFV1 = case x of
    Right ty ->
      getAll
        ( Lens.foldMapOf
            (typeFreeVars' isLocallyBound mempty)
            (const (All False))
            ty
        )
    Left tm ->
