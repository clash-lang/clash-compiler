          ...
          Just (BlackBox pNm _ _ _ _ _ _ _ _ _ inc r ri templ) ->
            Just $
              if
                | isFromInt pNm -> [0, 1, 2]
                | primName pInfo
                    `elem` [ Text.showt 'dontApplyInHDL,
  ...
  where
    ...
    go isN (id1, collectTicks -> (Letrec binds1 body1, ticks)) = do
      ...
      let (binds2, body2, isN1) =
            ...
            if any (`elemInScopeSet` isN) bs1
              then case deShadowTerm isN (Letrec binds1 body1) of
                Letrec bindsN bodyN ->
      ...
      (isN1,) . map (second (`mkTicks` nmTicks)) <$> case binds2 of
        ...
        -- the body
        [(id2, e2)]
          | Just occ <- lookupVarEnv id2 bodyOccs,
            e2WorkFree || occ < 2 ->
              if id2 `elemFreeVars` e2
                -- Except when the binder is recursive!
                then changed ([(id2, e2), (id1, body2)])
                else
                  let subst = extendIdSubst (mkSubst isN1) id2 e2
                   in changed
                        [ ( id1,
                            -- Only apply srcTicks to the body
                            mkTicks
                              (substTm "flattenLetGo" subst body2)
                              srcTicks
                          )
                        ]
        bs ->
    ...
    Right (args, bndrs, res) -> do
      ...
      case (toInline, others) of
        (_ : _, _ : _) -> do
          ...
          let substsInline =
                extendIdSubstList (mkSubst is1) $
                  map (second (const resV)) toInline
              others' =
                map
                  (second (substTm "recToLetRec" substsInline))
                  others
...
isClassConstraint (tyView -> TyConApp nm0 _) =
  if
    -- Constraint tuple. The host module is 'GHC.Classes' on GHC <= 9.12 and
