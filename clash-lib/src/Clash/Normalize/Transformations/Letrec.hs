...
-- continues the spine (an @AppFun@, @TyAppC@, or @TickC@ context); the shape
-- dispatch only routes spine nodes (@App@, @TyApp@, @Prim@, @Tick@) here, so no
-- explicit node-shape check is needed. The single-alternative-Case handling
-- lives in 'removeUnusedExprCase' and is unaffected: the spine only threads
-- through applications, type applications, and ticks, so a Case is never an
-- inner node of a spine.
