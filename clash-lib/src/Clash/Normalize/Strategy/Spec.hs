...
-- | Normalisation transformation. The argument is the name of the compiled
-- 'Clash.Normalize.Strategy.constantPropagation' binding.
normalizationSpec :: Name -> Strat
normalizationSpec constantPropagationName =
  rmDeadcode >-> multPrim >-> callStrategy constantPropagationName >->
...
constantPropagationSpec =
  ...
  where
    ...
    -- new root.
    --
    -- NB: 'topdownFix' is deliberately _not_ used here, see
    -- Note [topdownFixR is not for inlining bundles].
    inlineAndPropagate = repeatR (topdown transPropagateAndInline >-> inlineNR)
...
1. The evaluator-backed rewrites ('reduceConst', 'reduceNonRepPrim') must not
   sit inside the top-down propagation chain. 'topdownFix' settles that chain
   at each node until a fixpoint, so a chained 'reduceConst' is re-attempted -
...
2. There should be exactly two traversals per round, and the bottom-up one has
   ...
   A traversal rebuilds every node it walks, so each extra traversal costs a
   full term rebuild per round even when nothing fires. Running 'flattenLet'
   and the evaluator-backed rewrites as one fused bottom-up pass instead of two
