...
constantPropagation =
  ...
  where
    ...
    -- The propagate-and-inline bundle: one flat, ordered list. The
    -- per-constructor dispatch groups are derived by 'compileBundle', so they
    -- cannot fall out of sync with this order.
