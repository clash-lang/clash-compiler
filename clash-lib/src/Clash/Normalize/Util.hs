...
import Clash.Driver.Types (
  Binding (..),
  BindingMap,
  TransformationInfo (FinalTerm),
  hasTransformationInfo,
 )
...
mergeCsrs ctx ticks oldTerm proposedTerm subTerms = do
  ...
  if anyArgsOrResultConstant
    then
      let newTerm = proposedTerm (bimap csrNewTerm id <$> subCsrs)
       in pure
            ( ConstantSpecInfo
...
constantSpecInfo ctx e = do
  ...
  if isClockOrReset tcm (inferCoreTypeOf tcm e)
    then case collectArgs e of
      (Prim p, _)
        | primName p == Text.showt 'removedArg ->
            pure (constantCsr e)
      _ -> bindCsr ctx e
    else case collectArgsTicks e of
  ...
  go args changed (Lam v e)
    ...
    , Right tv `elem` args =
        let
         ...
         in
          go
            (Left (substId subst0 v) : (args List.\\ [Right tv]))
            True
            (substTm "substWithTyEq e" subst1 e)
  ...
  go argsOut changed (tyView -> FunTy arg tyRes)
    ...
    , Just (tv, ty) <- tvFirst tcArgs =
        let
         ...
         in
          go argsOut2 True (substTy subst tyRes)
...
rewriteExpr (nrwS, nrw) (bndrS, expr) (nm, sp) = do
  ...
  let expr' =
        traceIf
          (hasTransformationInfo FinalTerm opts)
          (bndrS ++ " before " ++ nrwS ++ ":\n\n" ++ before ++ "\n")
          expr
