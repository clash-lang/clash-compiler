...
extractHeadTail consCon elTy n vec =
  case dataConInstArgTys consCon tys of
    Just [coTy, _elTy, restTy] ->
      let mTV = mkTyVar typeNatKind (mkUnsafeSystemName "m" 0)
        ...
        [nilCon, consCon] <- tyConDataCons vecTc =
          if n == 0
            then
              ...
              let (a, as) = extractHeadTail consCon lhsElTy n lhsArg
               ...
               in mkVecCons consCon resElTy n c cs
    go _ ty =
      error $
        $(curLoc)
          ++ [I.i|
        ...
        [nilCon, consCon] <- tyConDataCons vecTc =
          if n == 0
            then
              ...
              let nPredTy = Right (LitTy (NumTy (n - 1)))
               ...
               in mkVecCons consCon resElTy n b bs
    go _ ty =
      error $
        $(curLoc)
          ++ [I.i|
...
reduceIterateI n aTy vTy _kn f0 a (TransformContext is0 ctx) = do
  ...
  uniqs0 <- Lens.use uniqSupply
  let is1 = extendInScopeSetList is0 (collectTermIds f1)
  ...
  let elems = map (App f1) (a : map Var elementIds)
    ...
    go tcm (tyView -> TyConApp vecTcNm _)
      | nameOcc vecTcNm == showt ''Vec,
        ...
        [_nilCon, consCon] <- tyConDataCons vecTc =
          let (a, as) = extractHeadTail consCon aTy n arg
              b =
                mkApps
                  (Prim foldrPrimInfo)
                  [ Right aTy,
          ...
          if n == 0
            then
              ...
              let nPredTy = Right (LitTy (NumTy (n - 1)))
               ...
               in mkVecCons consCon aTy n a as1
    go _ ty =
      error $
        $(curLoc)
          ++ [I.i|
