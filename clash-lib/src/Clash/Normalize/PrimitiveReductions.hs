...
vecHeadPrim vecTcNm =
  -- head :: Vec (n+1) a -> a
  Prim
    (PrimInfo "Clash.Sized.Vector.head" (vecHeadTy vecTcNm) WorkNever SingleResult NoUnfolding)
...
vecLastPrim vecTcNm =
  ...
  -- definition here.
  Prim
    (PrimInfo "Clash.Sized.Vector.last" (vecHeadTy vecTcNm) WorkNever SingleResult NoUnfolding)
...
vecTailPrim vecTcNm =
  ...
  Prim
    (PrimInfo "Clash.Sized.Vector.tail" (vecTailTy vecTcNm) WorkNever SingleResult NoUnfolding)
...
vecInitPrim vecTcNm =
  ...
  -- definition here.
  Prim
    (PrimInfo "Clash.Sized.Vector.init" (vecTailTy vecTcNm) WorkNever SingleResult NoUnfolding)
  ...
  go tcm (tyView -> TyConApp vecTcNm _)
    ...
    , [nilCon, consCon] <- tyConDataCons vecTc =
        do
          ...
          changed lb
  go _ ty =
    error $ $(curLoc) ++ "reduceReverse: argument does not have a vector type: " ++ showPpr ty
...
-}
reduceZipWith ::
  -- | zipWith primitive info
  PrimInfo ->
  -- | Length of the vector(s)
  Integer ->
  -- | Element type of the lhs of the function
  Type ->
  -- | Element type of the rhs of the function
  Type ->
  -- | Element type of the result of the function
  Type ->
...
reduceImap ::
  ...
  -- | Result type of the function
  Type ->
  -- | Lenght of the vector (as a KnownNat)
  Term ->
  -- | The imap'd function
  Term ->
          ...
          let is1 = extendInScopeSetList is0 (collectTermIds fun1)
              ...
              idxs =
                map
                  ( App
                      ( App
                          (TyApp idxFromInteger (LitTy (NumTy n)))
          ...
          (uniqs1, apDictIds@[functorDictId, pureId, apId, _, _, _]) <-
            pure $
              mapAccumR
                ...
                (uniqs0, is1)
                ( zipEqual
                    ["functorDict", "pure", "ap", "liftA2", "apConstL", "apConstR"]
                    apDictIdTys
                )
...
mkTravVec vecTc nilCon consCon pureTm apTm fmapTm bTy = go
 where
  ...
  go _ [] =
    mkApps
      pureTm
      [ Right (mkTyConApp vecTc [LitTy (NumTy 0), bTy])
      , Left
          ( mkApps
              (Data nilCon)
              [ Right (LitTy (NumTy 0))
    ...
    mkApps
      ...
      , Left
          ( mkApps
              ...
              , Right
                  ( mkFunTy
                      (mkTyConApp vecTc [LitTy (NumTy (n - 1)), bTy])
  ...
  consCoTy n = case dataConInstArgTys
    consCon
    [ (LitTy (NumTy n))
    , bTy
    , (LitTy (NumTy (n - 1)))
...
reduceFold n aTy fun arg (TransformContext is0 ctx) = do
 ...
 where
  ...
  foldV _ [a] = a
  foldV f as =
    let (l, r) = splitAt (length as `div` 2) as
        lF = foldV f l
        rF = foldV f r
        ...
        do
          ...
          let is1 = extendInScopeSetList is0 (collectTermIds fun)
              ...
              ubp k = Maybe.fromMaybe
                (error "reduceDFold: failed to extract upper bound proof")
                $ do
          ...
          changed lb
  go _ ty =
    error $ $(curLoc) ++ "reduceDFold: argument does not have a vector type: " ++ showPpr ty

  doFold _ _ _ [] = start
  doFold ubp snDc k (x : xs) =
    mkApps
      fun
      [ Right (LitTy (NumTy k))
      , Left (Data (ubp k))
  ...
  go tcm (tyView -> TyConApp vecTcNm _)
    ...
    , [_, consCon] <- tyConDataCons vecTc =
        do
          ...
          changed lb
  go _ ty =
    error $ $(curLoc) ++ "reduceAppend: argument does not have a vector type: " ++ showPpr ty
...
reduceUnconcat ::
  ...
  -- | Element type
  Type ->
  -- | Length of the result vector (as a KnownNat)
  Term ->
  -- | SNat "Length of the elements of the result vector"
  Term ->
  -- | Argument vector
  Term ->
  TransformContext ->
...
reduceTranspose ::
  ...
  -- | Element type
  Type ->
  -- | Lenght of the result vector (as a KnownNat)
  Term ->
  -- | Argument vector
  Term ->
  TransformContext ->
  ...
  go tcm (tyView -> TyConApp vecTcNm _)
    ...
    , [_, consCon] <- tyConDataCons vecTc =
        do
          ...
          changed lb
  go _ ty =
    error $ $(curLoc) ++ "reduceDTFold: argument does not have a vector type: " ++ showPpr ty
...
reduceTFold ::
  ...
  Type ->
  -- | Depth of the tree (as a KnownNat)
  Term ->
  -- | The motive
  Term ->
  -- | Function to convert elements with
