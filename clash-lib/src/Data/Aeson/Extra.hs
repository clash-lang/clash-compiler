...
genLineErr' allLines range errorLines =
  T.unlines
    [ T.concat
        [ if elem i errorLines then pack ">> " else pack "   "
        , pack $ show (i + 1)
        , pack ". "
        , allLines !! i
        ]
    ...
    Right (!lineMap, !contents1) ->
      case parse json contents1 of
        ...
        Fail bytes cntxs msg ->
          clashError
            ( "Could not read or parse json in "
                ++ show path
                ++ ". "
