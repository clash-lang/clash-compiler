...
-- | An "empty" 'NormalizeState': empty caches, and the strategies a
-- 'NormalizeSession' needs to normalize other binders.
defNormalizeState :: IO NormalizeState
defNormalizeState =
  NormalizeState
    <$> newMVar emptyVarEnv
    <*> newMVar Map.empty
    ...
    <*> newMVar emptyVarEnv
    <*> pure normalization
    <*> pure constantPropagation

-- | An "empty" 'RewriteState' carrying the given extra state.
defRewriteState :: extra -> IO (RewriteState extra)
defRewriteState extraState =
  RewriteState
    <$> newMVar mempty
    <*> newMVar mempty
    <*> newMVar emptyVarEnv
...
runSingleTransformationDef rewrite term = do
  st <- defRewriteState =<< defNormalizeState
