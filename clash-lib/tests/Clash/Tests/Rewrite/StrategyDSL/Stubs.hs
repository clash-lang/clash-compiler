{-|
  ...
  "Clash.Tests.Rewrite.StrategyDSL". They live in their own module because
  Template Haskell splices can only evaluate imported values: the test module
  splices 'Clash.Rewrite.StrategyDSL.TH.compileStrategy' over the 'Step's and
...
--
-- Each stub is a worker plus its 'Transformation'; some also come with a
-- hand-written 'Rewrite' doing its own constructor match, to compare the
-- compiled dispatch against.
...
collapseApplication :: Transformation
collapseApplication =
  toTransformation "collapseApplication" (onApp 'collapseApplicationWorker)
...
-- 'App', the way spine transformations are registered.
...
collapseSpine :: Transformation
collapseSpine =
  toTransformation "collapseSpine" (onAppNode 'collapseSpineWorker)
...
incrementOdd :: Transformation
incrementOdd = toTransformation "incrementOdd" (onLiteral 'incrementOddWorker)

-- | Like 'incrementOdd', but constructor-agnostic: a plain rewrite offered at
...
incrementOddAny :: Transformation
incrementOddAny = anyConstructor "incrementOddAny" 'incrementOddAnyWorker

-- | Multi-constructor: increments an odd integer literal, either as the
-- argument of an application or bare. Registered for App and Literal, to
-- observe the guarded suffix.
...
incrementBoth :: Transformation
incrementBoth = toTransformation "incrementBoth"
...
inlineTrivialLet :: Transformation
inlineTrivialLet =
  toTransformation "inlineTrivialLet" (onLet 'inlineTrivialLetWorker)
...
reifyContext :: Transformation
reifyContext = toTransformation "reifyContext" (onVar 'reifyContextWorker)
...
functionToPrim :: Transformation
functionToPrim = toTransformation "functionToPrim" (onVar 'functionToPrimWorker)
...
constructorChangingStep :: Step
constructorChangingStep =
  chain [collapseApplication, incrementBoth, inlineTrivialLet]
...
anyWorkerStep :: Step
anyWorkerStep = chain [collapseApplication, incrementOddAny]

nodeWorkerStep :: Step
nodeWorkerStep = chain [collapseSpine, incrementBoth]

-- | A cross-constructor conditional between chain segments inside one
-- traversal, the @flatten@ strategy's structure.
...
-- | A strategy calling another compiled strategy, parameterized over the
-- callee so the test module can point it at either compilation of it.
callStrategyStrat :: Name -> Strat
callStrategyStrat callee = callStrategy callee >-> topdown collapseApplication

-- | Every fused traversal of a 'Step' next to its unfused reference:
-- @[(name, fast, reference)]@. Spliced by the test module.
allTraversals :: Step -> Q Exp
allTraversals step = listE
  [ [| ($(stringE name), $(compileStrategy strat), $(compileStrategyReference strat)) |]
  | (name, strat) <-
