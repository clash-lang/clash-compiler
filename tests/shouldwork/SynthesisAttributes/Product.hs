...
topEntity ::
  ...
  Signal
    System
    ( Signed 9 `Annotate` 'StringAttr "top" "input1"
    , Signed 9 `Annotate` 'StringAttr "top" "input2"
    ) ->
  Signal
    System
    ( Signed 9 `Annotate` 'StringAttr "top" "output1"
...
testBench = done'
 where
  ...
  expectedOutput a =
    ...
    withClockResetEnable (tbSystemClockGen (not <$> done')) systemResetGen enableGen done
