...
-- Before https://github.com/clash-lang/clash-compiler/pull/1354 changing Lanes
...
catLayout ::
  ...
  BitLayout (Vec m s) ->
  BitLayout (Vec (n + m) s)
catLayout (BitLayout aBitCount aInFn aOutFn) (BitLayout bBitCount bInFn bOutFn) =
  let
    inFn = splitSigFn >>> (aInFn <=> bInFn) >>> catSigFn
    outFn = splitSigFn >>> (aOutFn <=> bOutFn) >>> catSigFn
   in
    BitLayout SNat inFn outFn
