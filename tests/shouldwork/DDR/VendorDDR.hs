...
topEntityGeneric vendorIn vendorOut clk rst en inIn outIn =
  ( bundle (genInOut, vendorInOut)
  , bundle (genOutOut, vendorOutOut)
  )
 where
  genInOut = ddrIn clk rst en (0, 0, 0) inIn
  vendorInOut = vendorIn clk rst en inIn
...
genOutSyncReset :: [D] -> [D]
genOutSyncReset es = P.take 13 es P.++ P.replicate 6 0 P.++ P.drop 19 es

xilinxOutSyncReset :: [D] -> [D]
xilinxOutSyncReset es = P.take 12 es P.++ P.replicate 7 0 P.++ P.drop 19 es

outAsyncReset :: [D] -> [D]
outAsyncReset es = P.take 11 es P.++ P.replicate 8 0 P.++ P.drop 19 es

genOutEnable :: [D] -> [D]
genOutEnable es = P.take 31 es P.++ P.take 8 (cycle [114, 214]) P.++ P.drop 39 es

xilinxOutEnable :: [D] -> [D]
xilinxOutEnable es = P.take 30 es P.++ P.replicate 9 114 P.++ P.drop 39 es

expIn :: [(D, D)]
expIn = (0,0) : (0,0) : P.zip [1, 3 .. 46] [2, 4 .. 46]

inSyncReset :: [(D, D)] -> [(D, D)]
inSyncReset es = P.take 7 es P.++ P.replicate 3 (0, 0) P.++ P.drop 10 es

inAsyncReset :: [(D, D)] -> [(D, D)]
inAsyncReset es = P.take 6 es P.++ P.replicate 4 (0, 0) P.++ P.drop 10 es

inEnable ::[(D, D)] ->  [(D, D)]
inEnable es = P.take 16 es P.++ P.replicate 4 (27, 28) P.++ P.drop 20 es
...
testBenchGeneric top expIn0 expGenOut expVendorOut =
 ...
 where
  ...
  ((genInOut1, genInOut2), (vendorInOut1, vendorInOut2)) =
    bimap unbundle unbundle $ unbundle inOut
  ...
  --
  -- Note that @IntelDDR.hs@ still suffers from the issue described in TODO
  --
  -- The `const` is just there to prevent a @-Wunused-matches@ on @expGenOut@.
  done1 = pure $ const True expGenOut
#endif
