...
busSwitch2 vec addr inp = zipWith ($) vec r
 where
...
topEntity = exposeClockResetEnable go
 where
  go (i, s) = bundle $ busSwitch1 v i s
   where
    v =
      f (+ 1) -- if address == 0 increment 1
 ...
 where
  testInput =
    stimuliGenerator clk rst $
      ( listToVecTH
          ([(a, Just b) | b <- [1, 2, 3, 4], a <- [0, 1, 2, 3]] :: [(Index 4, Maybe (Signed 5))])
      )
  expectedOutput =
    outputVerifier' clk rst $
      ( listToVecTH
          [ $ (listToVecTH [Just 2 :: Maybe (Signed 5), Nothing, Nothing, Nothing])
