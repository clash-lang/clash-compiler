...
testInput ::
  ...
  Signal Dom2 (Unsigned 8)
testInput clkA rstA =
  unbundle . stimuliGenerator clkA rstA $
...
expectedOutput ::
  ...
  Signal Dom7 (Unsigned 8) ->
  Signal Dom7 Bool
expectedOutput clkB rstB =
  outputVerifierR clkB rstB 1 $
    4 :> 7 :> 10 :> Nil
...
outputVerifierR clk rst iR samples i0 =
  let t1 = snatToNum (clockPeriod @circuitDom)
   ...
   in -- Only assert while not finished
      mux f' f' $ assert clk rst "outputVerifier" i1 e f'
 where
