...
import Clash.Explicit.Prelude
  ...
  , NFDataX, errorX, unsafeSynchronizer, enableGen, delay, toEnable,
  (.&&.), unpack
  )
...
xpmCdcHandshake# initVals srcStages dstStages clkSrc clkDst srcIn srcSend dstAck =
 ...
 where
  srcOpts = XpmCdcSingleConfig{stages=srcStages, initialValues=initVals, registerInput=False}
  srcSendFfSynced = xpmCdcSingleWith srcOpts clkSrc clkDst srcSendFf

  dstOpts = XpmCdcSingleConfig{stages=dstStages, initialValues=initVals, registerInput=False}
  srcRcv = xpmCdcSingleWith dstOpts clkDst clkSrc dstAck
  ...
  dstOut = delay clkDst dstOutEna (initVal (unpack 0)) (unsafeSynchronizer clkSrc clkDst srcHsDataFf)
  dstReq = delay clkDst enableGen (initVal False) srcSendFfSynced

  initVal :: forall x . x -> x
  initVal v
    | initVals = v
    | otherwise = errorX "xpmCdcHandshake: initial values undefined"
