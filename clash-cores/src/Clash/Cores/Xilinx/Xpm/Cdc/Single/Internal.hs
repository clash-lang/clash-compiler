...
import Clash.Explicit.Prelude
  ( type (<=), SNat, Clock, KnownDomain, BitPack(BitSize), errorX
...
xpmCdcSingle# ::
  ...
  Signal src a ->
  Signal dst a
xpmCdcSingle# registerInput initValuesSupported stages clkSrc clkDst input
  | registerInput = go (snatToNum stages) (initVal :- input)
  | otherwise     = go (snatToNum stages) input
 where
  initVal
    | initValuesSupported = unpack 0
    | otherwise = errorX "xpmCdcSingle: initial values undefined"
