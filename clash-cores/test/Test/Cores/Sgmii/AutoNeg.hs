...
autoNegSim ::
  ...
  C.Signal dom (ConfReg, SyncStatus, Maybe Rudi, Maybe ConfReg) ->
...
prop_autoNegNoAcknowledgeComplete = H.property $ do
  ...
  let simOut =
       ...
       where
        f i = (0, Ok, Nothing, Just i)

  H.assert $ isNothing (find g simOut) && isJust (find h simOut)
 where
