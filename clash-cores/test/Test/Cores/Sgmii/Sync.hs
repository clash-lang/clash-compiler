...
prop_syncNotOk = H.property $ do
  ...
  let simOut =
        map f $
          C.sampleN (simDuration + 1) (sync @C.System (C.fromList (0 : inp)))
       where
        ...
        map f $
          drop delaySamples $
            C.sampleN
              ...
              (sync @C.System (C.fromList (replicate delaySamples 0 ++ inp)))
       where
        f (_, _, _, rxEven, _) = rxEven

  H.assert $
    [Odd, Odd] `notElem` group simOut
      && length (maximumBy (compare `on` length) (group simOut)) < 3
