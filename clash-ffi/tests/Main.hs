    ...
    testGroup
      ...
      [ testGroup
          ...
          [ testProperty "registerCallback (vpi_register_cb)" $
              testM $
                sendReceiveAndCompare @(CallbackInfo BSNT) registerCallback
      ...
      , testGroup
          ...
          [ testProperty "controlSimulator (vpi_control)" $
              testM $
                sendAndCompare controlSimulator
          ]
      , testGroup
          "Clash.FFI.VPI.Error"
          [ testProperty "receiveErrorLevel (vpi_chk_error)" $
              testM $
                receiveAndCompare receiveErrorLevel
          , testProperty "unsafeReceiveErrorInfo (vpi_chk_error)" $
              testM $
                receiveAndCompare unsafeReceiveErrorInfo
          , testProperty "receiveErrorInfo (vpi_chk_error)" $
              testM $
                receiveAndCompare receiveErrorInfo
          ]
      , testGroup
          "Clash.FFI.VPI.IO"
      ...
      , testGroup
          ...
          [ testProperty "freeObject (vpi_free_object)" $
              testM $
                sendAndCompare @Object freeObject
          , testProperty "compareObjects (vpi_compare_objects)" $
              testM $
                sendReceiveAndCompare2 @Object @Object compareObjects
              ...
              testM $ \(prop :: Property CString) ->
                specialTop
                  >>= sendReceiveAndCompare2
                    (((SerialBS <$>) .) . receiveProperty)
                    . (prop,)
          , testProperty "receiveTime (vpi_get_time)" $
              testM $
                sendReceiveAndCompare2 @_ @(Maybe Object) receiveTime
