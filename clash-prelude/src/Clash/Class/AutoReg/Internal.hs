...
constraintsWantedFor clsNm [ty] = case ty of
  ...
  ConT _ -> return []  -- see [NOTE: No constraints for ConT]
  _ -> do
...
-- [NOTE: No constraints for ConT]
-- We don't emit constraints ConT's, because either these instances exist or they don't.
-- The ConT case here is for cases like:
--    data Message = Message Int deriving Generic
--    instance NFDataX Extra => NFDataX Message
--    deriveAutoReg ''Message
-- deriveAutoReg here will generate an unconstrained @instance AutoReg Message@
-- This works when the @instance NFDataX Extra@ exists,
-- or generates a compile error when it doesn't exist.
-- If we were to emit a constrained @instance NFDataX Extra => AutoReg Message@,
-- we'd just move the error to where the generated instance is used.
