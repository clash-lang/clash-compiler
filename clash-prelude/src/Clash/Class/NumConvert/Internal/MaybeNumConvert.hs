...
import Clash.Class.NumConvert.Internal.Through (Through)
...
'maybeNumConvertThrough' resolves this by explicitly routing through canonical forms:

>>> maybeNumConvertThrough (42 :: Int64) :: Maybe Word64
Just 42
>>> maybeNumConvertThrough (-1 :: Int64) :: Maybe Word64
Nothing
...
maybeNumConvertThrough ::
  forall a b.
  ( NumConvert a (Through a)
  , MaybeNumConvert (Through a) (Through b)
  , NumConvert (Through b) b
  ) =>
  a ->
  Maybe b
maybeNumConvertThrough a =
    fmap (numConvert @(Through b) @b)
  $ maybeNumConvert @(Through a) @(Through b)
  $ numConvert @a @(Through a) a
