  ...
  USucc _ -> case rstStrategy of
    ClearOnReset initF ->
      ...
      where
        ...
        -- Note the use of 'fromEnum' instead of 'fromInteger . toInteger':
        -- 'Clash.Sized.Internal.Index.fromEnum#' has a black box, so 'Integer'
        -- does not end up in the generated HDL. 'numConvert' cannot be used
        -- here, as its 'Index n -> Int' instance would constrain @n@.
...
blockRam1 clk rst0 en rstStrategy n@SNat a rd0 mw0 = case toUNat n of
 ...
 where
  ...
  rd1 = mux rstBool 0 (fromEnum <$> rd0)
  we1 = mux rstBool (pure True) we0
  -- See the note on 'wa1' in 'blockRamU' on why 'fromEnum' is used here
  wa1 = mux rstBool (fromEnum <$> waCounter) (fromEnum <$> wa0)
