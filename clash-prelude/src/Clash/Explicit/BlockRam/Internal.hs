...
import Data.ByteString.Builder (Builder, toLazyByteString, word64BE, word8)
...
packBVs care es =
  case lenOrErr of
    Nothing -> Left err
    Just len ->
      let (runs, ends) = packAsNats mI (knownBVVal . pack) es
...
packAsNats width trans es = (toLazyByteString runs0, toLazyByteString ends)
 where
  ...
  packRun runC val1 runs1 =
    let (val2, runB) = val1 `divMod` 256
  ...
  packEnd val2 ends1 endC1 endA1
    ...
    | endC2 <= 64 =
        let endA2 = endA1 * (2 ^ endL) + valEnd
         in (ends1, endC2, endA2)
    | otherwise =
        let ends2 = word64BE endA1 <> ends1
...
unpackW64s = go 8 0
 where
  ...
  go n val endBs =
    let (endB, endBs0) = case B.uncons endBs of
