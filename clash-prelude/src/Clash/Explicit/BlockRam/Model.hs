...
import Clash.XException (NFDataX (..), XException (..), seqX)
import Clash.XException.MaybeX (MaybeX (..), andX, toMaybeX)
...
cycleOne SNat TdpbramModelConfig {..} prev ram0 = \case
  ...
  (ena, addr, byteEna0, dat) ->
    let byteEna1 = tdpMergeWriteEnable ena byteEna0
        (out0, !ram1) =
...
accessRam SNat tdpIsActiveWriteEnable updateMem addrX byteEnableX dat mem0
  ...
  where
    ...
    unknownAddr msg n =
      deepErrorX
        ( "Write enabled or undefined, but address unknown; position "
            <> show n
