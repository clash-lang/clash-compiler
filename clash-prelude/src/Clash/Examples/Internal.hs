...
lfsrG seed = last (unbundle r)
 where
  r = register (unpack seed) (lfsrGP (unpack 0b0011010000000000) <$> r)
...
grayCounter en = gray <$> upCounter en
 where
  gray xs = pack (msb xs) ++# xor (slice d7 d1 xs) (slice d6 d0 xs)
