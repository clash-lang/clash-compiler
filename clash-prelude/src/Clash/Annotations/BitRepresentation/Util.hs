 ...
 where
  ...
  bitOrigin n =
    if testBit mask n
      ...
      else case findIndex (\fmask -> testBit fmask n) fields of
        Nothing ->
          if testBit commonMask n
            then
        ...
        Just fieldn ->
          let fieldbitn =
                length $
                  filter id $
                    take n $
                      bitsToBools (fields !! fieldn)
           in Field fieldn fieldbitn fieldbitn
...
bitsToBools 0 = []
bitsToBools n
  | n < 0 = error "Can't deal with negative bitmasks/values"
