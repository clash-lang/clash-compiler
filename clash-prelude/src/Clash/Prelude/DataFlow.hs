...
module Clash.Prelude.DataFlow
  {-# DEPRECATED
    "Module will be removed in Clash 1.10 in favor of clash-protocols. See: https://github.com/clash-lang/clash-protocols/."
    #-} (
  -- * Data types
  DataFlow (..),
...
import Clash.Explicit.Signal (Clock, Enable, Reset, Signal, andEnable, register)
...
fifoDF_mealy (mem, rptr, wptr) (wdata, winc, rinc) =
  let raddr = truncateB rptr :: BitVector addrSize
      ...
      mem'
        | winc && not full = replace waddr wdata mem
        | otherwise = mem
...
fifoDF ::
  ...
  SNat (m + n) ->
  {- | Initial content. Can be smaller than the size of the
...
instance (LockStep a x, LockStep b y) => LockStep (a, b) (x, y) where
  lockStep =
    (lockStep `parDF` lockStep)
      `seqDF` ( DF
                  ( \xy xyV rdy ->
                      let (xV, yV) = unbundle xyV
                          val = xV .&&. yV
                          xR = yV .&&. rdy
                          yR = xV .&&. rdy
                          xyR = bundle (xR, yR)
