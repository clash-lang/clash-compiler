...
import Language.Haskell.TH (Q, Quote, appT, conT, litT, mkName, numTyLit, sigE)
...
import Clash.Promoted.Nat (SNat, natToInteger, natToNum)
...
instance
  ...
  show f@(Fixed fRep) =
    i
      ++ "."
      ++ ( uncurry pad
             . second (show . numerator)
             . fromJustX
             . find ((== 1) . denominator . snd)
             . iterate (succ *** (* 10))
             . (,) 0
             $ (nom % denom)
         )
...
instance
  (size ~ (int + frac), KnownNat frac, Bounded (rep size), Integral (rep size)) =>
  Read (Fixed rep int frac)
  where
...
-}
instance
  (ENumFixedC rep int1 frac1 int2 frac2) =>
  ExtendingNum (Fixed rep int1 frac1) (Fixed rep int2 frac2)
  where
  type
...
instance
  (BitPack (rep (int + frac)), KnownNat (BitSize (rep (int + frac)))) =>
  BitPack (Fixed rep int frac)
  where
...
instance
  (Lift (rep (int + frac)), KnownNat frac, KnownNat int, Typeable rep) =>
  Lift (Fixed rep int frac)
  where
  lift f@(Fixed fRep) =
    sigE
      [|Fixed fRep|]
  ...
  succ f =
    let err =
     ...
     in case natToInteger @int of
          0 -> err
          _ ->
            if f > satPred SatBound maxBound
              then
                err
              else
                satSucc SatWrap f
  ...
  pred f =
    let err =
     ...
     in case natToInteger @int of
          0 -> err
          _ ->
            if f < satSucc SatBound minBound
              then
                err
              else
                satPred SatWrap f
...
enumFromThenToUp x1 x2 y
  | y < x1 =
      let y' = satAdd SatWrap y halfDelta -- Never wraps
       in if y' < x1 || (isMinusHalf && y' <= x1)
            then
              []
            else
              [x1]
  | y < x2 =
      let x2' = satSub SatWrap x2 halfDelta -- Never wraps `
       in if y > x2' || (not isMinusHalf && y >= x2')
            then
              [x1, x2]
            else
              [x1]
  | otherwise =
      let y' = satSub SatWrap y (delta `shiftR` 1) -- Does wrap
          go_up x
            | x' < x = [x]
            | isHalf && x >= y' = [x]
            | x > y' = [x]
...
enumFromThenToDown x1 x2 y
  | y > x1 =
      let y' = satSub SatWrap y halfDelta -- Never wraps
       in if y' > x1 || (isMinusHalf && y' >= x1)
            then
              []
            else
              [x1]
  | y > x2 =
      let x2' = satAdd SatWrap x2 halfDelta -- Never wraps `
       in if y < x2' || (not isMinusHalf && y <= x2')
            then
              [x1, x2]
            else
              [x1]
  | otherwise =
      let y' = satAdd SatWrap y (delta `shiftR` 1) -- Does wrap
          go_dn x
            | x' > x = [x]
            | isHalf && x <= y' = [x]
            | x < y' = [x]
  ...
  satMul SatBound (Fixed a) (Fixed b) =
    let res = a `mul` b
     ...
     in case isSigned a of
          True ->
            let overflow =
                  complement (reduceOr (pack (msb rR) ++# pack rL))
