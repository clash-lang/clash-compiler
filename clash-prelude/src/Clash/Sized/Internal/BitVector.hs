...
import GHC.Exts (Word (W#), Word#, dataToTag#, eqWord#, gtWord#, int2Word#, isTrue#, minusWord#, uncheckedShiftRL#, word2Int#)
...
import Language.Haskell.TH (Exp (AppE, LitE, SigE, VarE), ExpQ, Lit (..), Pat, Q, Quote, TyLit (NumTyLit), Type (AppT, ConT, LitT), appT, conT, litE, litP, litT, mkName, numTyLit, sigE, tupE, tupP, varP)
...
complement## (Bit m v) = Bit m (complementB v .&. complementB m)
 where
  complementB (W# b#) = W# (int2Word# (eqWord# b# 0##))
{-# OPAQUE complement## #-}
...
index# bv@(BV m v) i
  | i >= 0 && i < sz =
      Bit
        (W# (int2Word# (dataToTag# (testBit m i))))
...
lsb# (BV m v) =
  Bit
    (W# (int2Word# (dataToTag# (testBit m 0))))
...
replaceBit# bv@(BV m v) i (Bit mb b)
  | i >= 0 && i < sz =
      BV
        (clearBit m i .|. (naturalFromWord mb `shiftL` i))
...
truncateB# = \(BV msk i) -> BV (msk `mod` m) (i `mod` m)
 where
  m = 1 `naturalShiftL` naturalToWord (natVal (Proxy @a))
{-# OPAQUE truncateB# #-}
