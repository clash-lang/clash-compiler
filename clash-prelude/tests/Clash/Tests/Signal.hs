...
case_dynamicStaticEq = do
  let sampleMagicN = P.take 500 . sample
...
case_signalAutomatonFreshState = do
  let circuit :: (HiddenClockResetEnable dom) => Signal dom Int -> Signal dom Int
      circuit = register 0
...
case_changingDynamicClocks = do
  ...
  assertEqual
    "unsafeSynchronizer produced hardcoded results"
    actual
    [ 0,
      4,
      6,
