...
saturateToBounded Proxy satMode x =
  let repInt =
   ...
   in saturate
        (toRational (minBound @b))
        (toRational (maxBound @b))
        repInt
        satMode
        x
...
predProperty Proxy = property $ do
  ...
  if toRational x - 1 < toRational (minBound @f)
    then do
...
succProperty Proxy = property $ do
  ...
  if toRational x + 1 > toRational (maxBound @f)
    then do
...
fromRationalTowards to x
  ...
  | otherwise =
      let mb = maxBound :: Fixed rep int frac
...
enumFromProperty Proxy = property $ do
  let minVal =
        if toRational (maxBound @f) < maxLength
          then
            minBound
          else
            maxBound - maxLength
