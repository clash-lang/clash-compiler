...
import Hedgehog.Extra (ParallelWalk (..), combinations, parallelWalk)
...
saneTimestamps ::
  (H.MonadTest m) =>
  ParallelWalk m SimulationCommand
saneTimestamps = ParallelWalk $ fixedStart (0 :: Int) []
 where
  ...
  fixedStart 3 starts _ _ =
    failWithMsg $
      "Simulation doesn't start with proper $dumpvars\n\n"
        ...
        <> show starts
  fixedStart n starts False sc = pure $ ParallelWalk $ fixedStart (n + 1) (starts ++ [sc])
  fixedStart _ _ True _ = failWithMsg "Simulation ends prematurely"

  go _ t0 _ (SimulationValueChange _) = pure $ ParallelWalk $ go False t0
  go False t0 False (SimulationTime t1) = H.diff t0 (<) t1 >> pure (ParallelWalk $ go True t1)
...
signalCorrect ::
  ...
  [Value] ->
  ParallelWalk m SimulationCommand
signalCorrect name idCode period end es = ParallelWalk initF
 where
  initF _ (SimulationTime t) =
    pure $ ParallelWalk $ go t (zip (-1 : [0, period .. end]) es)
  initF _ sc =
  ...
  go t es0 lastC (SimulationValueChange (ValueChange _ idCode0 v))
    | idCode /= idCode0 = checkFinal lastC es0 >> pure (ParallelWalk $ go t es0)
    | otherwise = do
        es1 <- parseChange t es0 idCode0 v
        checkFinal lastC es1
        pure $ ParallelWalk $ go t es1
  go _ es0 _ (SimulationTime t) = pure $ ParallelWalk $ go t es0
      ...
      k <-
        H.forAll $
          Gen.shrink (\x -> if x > 1 then [1] else []) $
            Gen.int $
              Range.linear 2 4
      H.label (fromString $ "Signals: " <> show k) -- XXX: Remove
        ...
        signalCorrects =
          map
            ...
            $ Map.intersectionWith
              ...
              nameMap
              traceMap
      parallelWalk (saneTimestamps : signalCorrects) vcdSims0
 where
  oneTrace period =
