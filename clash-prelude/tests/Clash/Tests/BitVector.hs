...
import Clash.Sized.Internal.BitVector (Bit (..), BitVector (..), index#, replaceBit#)
import Clash.Tests.SizedNum
import Clash.XException (deepErrorX, ensureSpine, errorX)
import Data.Bits (Bits (rotateL, rotateR, shiftL, shiftR))
import Data.Proxy
...
fromIntegerRandomProp (Q.Positive m) n =
  m > 1 Q.==> case someNatVal (fromInteger m) of
