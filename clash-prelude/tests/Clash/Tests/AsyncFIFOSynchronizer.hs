...
sync12RW w1 w2 w3 =
 ...
 where
  ...
  inputRInc =
    fromList
      $ P.replicate w1 False
      <> P.replicate 4 True
      <> P.replicate w2 False
      <> P.replicate 4 True
      <> P.repeat False
  inputWDataM =
    fromList $ Nothing
      : P.map Just [1 .. 4]
        <> P.replicate w3 Nothing
        <> P.map Just [5 .. 8]
        <> P.repeat Nothing
...
sampleR n r =
  P.map
    (\(inputRInc, (rdata, rempty)) -> (inputRInc, (maybeIsX rdata, rempty)))
    (sampleN n r)
...
fifoOperations racts wacts = (bundle (rAllDone, rel), bundle (wAllDone, wel))
 where
  ...
  rel =
    mux
      (liftA3 (\en inc em -> en && inc && not em) ren rinc rempty)
      (Just <$> rdata)
      (pure Nothing)
  wel =
    mux
      (liftA3 (\en wdm fu -> en && isJust wdm && not fu) wen wDataM wfull)
      (Just <$> wdata)
      (pure Nothing)
...
tests =
  testGroup
    ...
    , testPropertyXXX "Functional test"
        $ forAllNamedTestProperties fifoFunctionalTestCombinations
    ]
