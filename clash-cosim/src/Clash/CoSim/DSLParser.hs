...
dslParser =
  let toCoSimDSL (name, tokens) = (Just $ name, tokens)
   in try (toCoSimDSL <$> withHeaderParser) <|> ((,) Nothing) <$> sourceParser
...
vars tokens =
  let varNames' = varNames tokens
   in (varNames', anonymousNames (nVarNums tokens) varNames')
...
anonymousNames nVarNums' varNames' =
  let aaName n m = (replicate m '_') ++ "aa" ++ show n
   in map (Inf.head . Inf.dropWhile (`elem` varNames')) $
...
toVerilog dsl moduleName =
  concat
    [ printf
        "module %s (%sresult);"
        moduleName
        (concatMap (++ ", ") $ uncurry (++) (clks dsl) ++ uncurry (++) (vars dsl))
    , "\n"
    , tokensToString dsl
    , "\n"
    , "endmodule"
