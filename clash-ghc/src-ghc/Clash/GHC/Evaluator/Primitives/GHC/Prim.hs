...
primitives =
  ...
  [ primStepEntry $(textNameLit 'GHC.Prim.gtChar#) $ \case
    ...
    primStepEntry $(textNameLit 'GHC.Prim.byteSwap#) $ \case
      PrimStepContext {..}
        | [i] <- wordLiterals' args -> -- assume 64bits
            reduce . integerToWordLiteral . toInteger . byteSwap64 . (fromInteger :: Integer -> Word64) $ i
      _ -> Nothing,
    ...
    primStepEntry $(textNameLit 'GHC.Prim.uncheckedShiftLInt16#) $ \case
      PrimStepContext {..}
        | Just r <- liftI16I uncheckedShiftLInt16# args ->
            reduce r
      _ -> Nothing,
    primStepEntry $(textNameLit 'GHC.Prim.uncheckedShiftRAInt16#) $ \case
      PrimStepContext {..}
        | Just r <- liftI16I uncheckedShiftRAInt16# args ->
            reduce r
      _ -> Nothing,
    primStepEntry $(textNameLit 'GHC.Prim.uncheckedShiftRLInt16#) $ \case
      PrimStepContext {..}
        | Just r <- liftI16I uncheckedShiftRLInt16# args ->
            reduce r
      _ -> Nothing,
    ...
    primStepEntry $(textNameLit 'GHC.Prim.uncheckedShiftLInt32#) $ \case
      PrimStepContext {..}
        | Just r <- liftI32I uncheckedShiftLInt32# args ->
            reduce r
      _ -> Nothing,
    primStepEntry $(textNameLit 'GHC.Prim.uncheckedShiftRAInt32#) $ \case
      PrimStepContext {..}
        | Just r <- liftI32I uncheckedShiftRAInt32# args ->
            reduce r
      _ -> Nothing,
    primStepEntry $(textNameLit 'GHC.Prim.uncheckedShiftRLInt32#) $ \case
      PrimStepContext {..}
        | Just r <- liftI32I uncheckedShiftRLInt32# args ->
            reduce r
      _ -> Nothing,
    ...
    primStepEntry $(textNameLit 'GHC.Prim.uncheckedIShiftL64#) $ \case
      PrimStepContext {..}
        | Just r <- liftI64I uncheckedIShiftL64# args ->
            reduce r
      _ -> Nothing,
    primStepEntry $(textNameLit 'GHC.Prim.uncheckedIShiftRA64#) $ \case
      PrimStepContext {..}
        | Just r <- liftI64I uncheckedIShiftRA64# args ->
            reduce r
      _ -> Nothing,
    primStepEntry $(textNameLit 'GHC.Prim.uncheckedIShiftRL64#) $ \case
      PrimStepContext {..}
        | Just r <- liftI64I uncheckedIShiftRL64# args ->
            reduce r
      _ -> Nothing,
    ...
    primStepEntry $(textNameLit 'GHC.Prim.uncheckedShiftLWord8#) $ \case
      PrimStepContext {..}
        | Just r <- liftW8I uncheckedShiftLWord8# args ->
            reduce r
      _ -> Nothing,
    primStepEntry $(textNameLit 'GHC.Prim.uncheckedShiftRLWord8#) $ \case
      PrimStepContext {..}
        | Just r <- liftW8I uncheckedShiftRLWord8# args ->
            reduce r
      _ -> Nothing,
    ...
    primStepEntry $(textNameLit 'GHC.Prim.uncheckedShiftLWord16#) $ \case
      PrimStepContext {..}
        | Just r <- liftW16I uncheckedShiftLWord16# args ->
            reduce r
      _ -> Nothing,
    primStepEntry $(textNameLit 'GHC.Prim.uncheckedShiftRLWord16#) $ \case
      PrimStepContext {..}
        | Just r <- liftW16I uncheckedShiftRLWord16# args ->
            reduce r
      _ -> Nothing,
    ...
    primStepEntry $(textNameLit 'GHC.Prim.uncheckedShiftLWord32#) $ \case
      PrimStepContext {..}
        | Just r <- liftW32I uncheckedShiftLWord32# args ->
            reduce r
      _ -> Nothing,
    primStepEntry $(textNameLit 'GHC.Prim.uncheckedShiftRLWord32#) $ \case
      PrimStepContext {..}
        | Just r <- liftW32I uncheckedShiftRLWord32# args ->
            reduce r
      _ -> Nothing,
    ...
    primStepEntry $(textNameLit 'GHC.Prim.uncheckedShiftL64#) $ \case
      PrimStepContext {..}
        | Just r <- liftW64I uncheckedShiftL64# args ->
            reduce r
      _ -> Nothing,
    primStepEntry $(textNameLit 'GHC.Prim.uncheckedShiftRL64#) $ \case
      PrimStepContext {..}
        | Just r <- liftW64I uncheckedShiftRL64# args ->
            reduce r
      _ -> Nothing,
                  ...
                  mkApps
                    ...
                    ( map Right tyArgs
                        ++ [ Left (Prim rwTy),
                             Left
                               ( mkApps
                                   (Prim (PrimInfo (showt ''MutableByteArray#) mbaTy WorkNever SingleResult NoUnfolding))
          ...
          [ba, i] <- intLiterals' [baV, iV] ->
            let (_, tyView -> TyConApp tupTcNm tyArgs) = splitFunForallTy ty
                ...
                ba2 = unsafeDupablePerformIO $ do
                  ...
                  mba' <-
                    IO
                      ( \s -> case resizeMutableByteArray# mba i' s of
          ...
          [ba, off] <- intLiterals' [baV, offV] ->
            let (_, tyView -> TyConApp tupTcNm tyArgs) = splitFunForallTy ty
                ...
                w = unsafeDupablePerformIO $ do
                  BA.MutableByteArray mba <- BA.unsafeThawByteArray ba1
                  IO
                    ( \s -> case readWordArray# mba off' s of
                        (# s', w' #) -> (# s', W# w' #)
                    )
                newE =
                  mkApps
