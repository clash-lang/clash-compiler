...
primitives =
  [ primStepEntry $(textNameLit 'GHC.Classes.eqInt) $ \case
      PrimStepContext {..}
        | Just (i, j) <- intCLiterals args ->
            reduce (boolToBoolLiteral tcm ty (i == j))
      _ -> Nothing,
    primStepEntry $(textNameLit 'GHC.Classes.neInt) $ \case
      PrimStepContext {..}
        | Just (i, j) <- intCLiterals args ->
            reduce (boolToBoolLiteral tcm ty (i /= j))
      _ -> Nothing,
    primStepEntry $(textNameLit 'GHC.Classes.leInt) $ \case
      PrimStepContext {..}
        | Just (i, j) <- intCLiterals args ->
            reduce (boolToBoolLiteral tcm ty (i <= j))
      _ -> Nothing,
    primStepEntry $(textNameLit 'GHC.Classes.ltInt) $ \case
      PrimStepContext {..}
        | Just (i, j) <- intCLiterals args ->
            reduce (boolToBoolLiteral tcm ty (i < j))
      _ -> Nothing,
    primStepEntry $(textNameLit 'GHC.Classes.geInt) $ \case
      PrimStepContext {..}
        | Just (i, j) <- intCLiterals args ->
            reduce (boolToBoolLiteral tcm ty (i >= j))
      _ -> Nothing,
    primStepEntry $(textNameLit 'GHC.Classes.gtInt) $ \case
      PrimStepContext {..}
        | Just (i, j) <- intCLiterals args ->
            reduce (boolToBoolLiteral tcm ty (i > j))
      _ -> Nothing,
    primStepEntry $(textNameLit '(GHC.Classes.&&)) $ \case
      PrimStepContext {..}
        | [lArg, rArg] <- args,
        ...
        | [lArg, rArg] <- args,
          ...
          mach2@Machine {mStack = [], mTerm = rArgWHNF} <- whnf eval tcm True (setTerm (valToTerm rArg) $ stackClear mach1) ->
            case [lArgWHNF, rArgWHNF] of
              ...
              _ -> Nothing
      _ -> Nothing,
    primStepEntry $(textNameLit 'GHC.Classes.divInt#) $ \case
      PrimStepContext {..}
    ...
    -- modInt# :: Int# -> Int# -> Int#
    primStepEntry $(textNameLit 'GHC.Classes.modInt#) $ \case
      PrimStepContext {..}
        | [dividend, divisor] <- intLiterals' args ->
            if divisor == 0
              then
                let iTy = snd (splitFunForallTy ty)
                 in reduce (TyApp (Prim NP.undefined) iTy)
              else
                reduce (Literal (IntLiteral (dividend `mod` divisor)))
      _ -> Nothing,
    primStepEntry $(textNameLit 'GHC.Classes.not) $ \case
      PrimStepContext {..}
        | [DC bCon _] <- args ->
