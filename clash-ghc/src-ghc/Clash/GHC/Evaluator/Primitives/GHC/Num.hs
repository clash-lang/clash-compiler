  ...
  [ primStepEntry $(textNameLit 'GHC.Num.naturalLogBase#) $ \case
      ...
      PrimStepContext {..}
        | Just (i, j) <- naturalLiterals args ->
            let nTy = snd (splitFunForallTy ty)
             in reduce (checkNaturalRange2 nTy i j (+))
      _ -> Nothing,
    ...
    primStepEntry $(textNameLit 'GHC.Num.naturalFromWord#) $ \case
      PrimStepContext {..}
        | [Lit (WordLiteral w)] <- args ->
            let nTy = snd (splitFunForallTy ty)
             in reduce (checkNaturalRange1 nTy w id)
      _ -> Nothing,
    ...
    primStepEntry $(textNameLit 'GHC.Num.naturalQuot) $ \case
      PrimStepContext {..}
        | Just (i, j) <- naturalLiterals args ->
            let nTy = snd (splitFunForallTy ty)
             in reduce (checkNaturalRange2 nTy i j quot)
      _ -> Nothing,
    ...
    primStepEntry $(textNameLit 'GHC.Num.naturalGcd) $ \case
      PrimStepContext {..}
        | Just (i, j) <- naturalLiterals args ->
            let nTy = snd (splitFunForallTy ty)
             in reduce (checkNaturalRange2 nTy i j gcd)
      _ -> Nothing,
    primStepEntry $(textNameLit 'GHC.Num.naturalLcm) $ \case
      PrimStepContext {..}
        | Just (i, j) <- naturalLiterals args ->
            let nTy = snd (splitFunForallTy ty)
             in reduce (checkNaturalRange2 nTy i j lcm)
      _ -> Nothing,
