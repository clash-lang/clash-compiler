...
import Clash.Core.Util (dataConInstArgTys, mkSelectorCase, mkVec, primCo, tyNatSize)
        ...
        | isSubj,
          ...
          Right n <- runExcept (tyNatSize tcm nTy) ->
            case n of
              ...
              _
                | _ : h' : t : _ <- Either.lefts vArgs,
                  (_, tyView -> TyConApp vecTcNm _) <- splitFunForallTy ty ->
                    reduceWHNF $
                      mkApps
                        (vecAppendPrim vecTcNm)
                        [ Right mTy,
                          Right aTy,
                          Right $
                            mkTyConApp
                ...
                | DC snatDc [_, Left d'] <- d,
                  ...
                  mach2@Machine {mStack = [], mTerm = Literal (NaturalLiteral d2)} <- whnf eval tcm isSubj (setTerm d' $ stackClear mach) ->
                    case (d2 `mod` n) of
                      ...
                      d3 ->
                        let (_, tyView -> TyConApp vecTcNm _) = splitFunForallTy ty
                            (Just vecTc) = UniqMap.lookup vecTcNm tcm
                            [nilCon, consCon] = tyConDataCons vecTc
                         in reduceWHNF' mach2 $
                              mkApps
      ...
      PrimStepContext {..} -- :: forall n a b . KnownNat n => (Index n -> a -> b) -> Vec n a -> Vec n b
        | isSubj,
          ...
          let iLit = mkIndexLit (Either.rights tyArgs' !! 0) nTy n 0 ->
            reduceWHNF $
              mkApps
                (Prim (PrimInfo "Clash.Sized.Vector.imap_go" (vecImapGoTy vecTcNm indexTcNm) WorkNever SingleResult NoUnfolding))
                [ Right nTy,
                  Right nTy,
                  Right aTy,
                  Right bTy,
                ...
                | eval <- Evaluator ghcStep ghcUnwind ghcPrimStep ghcPrimUnwind,
                  mach1@Machine {mStack = [], mTerm = n} <-
                    ...
                    let (tyArgs, _) = splitFunForallTy ty
                     ...
                     in Just
                          $ flip setTerm (mach1 {mStack = mStack mach})
                          $ mkVecCons
                            dc
                            bTy
                            m'
                            ...
                            ( mkApps
                                ...
                                [ Right nTy,
                                  ...
                                  Left
                                    ( mkApps
                                        (Prim (PrimInfo (showt '(Clash.Sized.Internal.Index.+#)) (indexAddTy indexTcNm) WorkVariable SingleResult NoUnfolding))
                                        [ Right nTy,
    ...
    primStepEntry $(textNameLit 'Clash.Sized.Vector.iterateI) $ \case
      PrimStepContext {..}
        | isSubj,
          ...
          [_n, f, a] <- args,
          Right n <- runExcept (tyNatSize tcm nTy) ->
            let TyConApp vecTcNm _ = tyView (getResultTy tcm ty tys)
                Just vecTc = UniqMap.lookup vecTcNm tcm
                [nilCon, consCon] = tyConDataCons vecTc
             in case n of
                  0 -> reduce (mkVecNil nilCon aTy)
            ...
            case n of
              ...
              _ ->
                let (tyArgs, _) = splitFunForallTy ty
                    ...
                    splitAtCall =
                      mkApps
                        (Prim (PrimInfo "Clash.Sized.Vector.fold_split" (foldSplitAtTy vecTcNm) WorkNever SingleResult NoUnfolding))
                        [ Right mTy,
                          Right n1mTy,
                          Right aTy,
      ...
      PrimStepContext {..}
        | isSubj,
          ...
          Right n <- runExcept (tyNatSize tcm nTy) ->
            case n of
              0 ->
                let (pureF, ids') = runPEM (mkSelectorCase $(curLoc) is0 tcm (valToTerm apDict) 1 1) ids
              ...
              _ ->
                let ((fmapF, apF), ids') = flip runPEM ids $ do
