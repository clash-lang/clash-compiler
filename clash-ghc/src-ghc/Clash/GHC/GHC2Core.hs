...
coreToTerm primMap unlocs = term
  where
    ...
    term' (Case _ _ ty [])  =
      C.TyApp (C.Prim (C.PrimInfo (pack "EmptyCase") C.undefinedTy C.WorkNever C.SingleResult Nothing))
        <$> coreToType ty
    ...
    term' (Type t) =
      C.TyApp (C.Prim (C.PrimInfo (pack "_TY_") C.undefinedTy C.WorkNever C.SingleResult Nothing))
        <$> coreToType t
    term' (Coercion co) =
      C.TyApp (C.Prim (C.PrimInfo (pack "_CO_") C.undefinedTy C.WorkNever C.SingleResult Nothing))
        ...
        case isDataConId_maybe x of
          Just dc -> case lookupPrim xNameS of
            Just p  ->
              ...
              return $ C.Prim (C.PrimInfo xNameS xType (maybe C.WorkVariable workInfo p) C.SingleResult Nothing)
          ...
          Nothing -> case lookupPrim xNameS of
            Just (Just (Primitive f wi _))
              ...
              | x `elem` unlocs
              -> return (C.Prim (C.PrimInfo xNameS xType wi C.SingleResult Nothing))
              | otherwise
              -> do bndr <- coreToId x
                    return (C.Prim (C.PrimInfo xNameS xType wi C.SingleResult (Just bndr)))
            ...
            Just Nothing ->
              ...
              return $ C.Prim (C.PrimInfo xNameS xType C.WorkVariable C.SingleResult Nothing)
            Nothing
              | x `elem` unlocs
              -> return (C.Prim (C.PrimInfo xNameS xType C.WorkVariable C.SingleResult Nothing))
              | otherwise
...
runRWTerm (C.ForAllTy rTV (C.ForAllTy oTV funTy)) =
  ...
  C.TyLam oTV (
  C.Lam   fId (
  (C.App (C.Var fId) (C.Prim (C.PrimInfo rwNm rwTy C.WorkNever C.SingleResult Nothing))))))
 where
