...
--
-- This note explains the why Clash implements some primitives, most notably
-- 'mapSignal#', 'appSignal#', and 'signal#' using Casts.
--
-- ==============
--
-- For user code, (de)constructing _Signal_s using Signal's constructor (-:) is
-- not synthesizable due to the ability to write non-causal properties. Functions
-- that need to (de)construct a Signal this way generally need a blackbox
-- implementation. This way, Clash will use the blackbox to produce HDL instead
-- of the Core generated by GHC. A number of primitives are excluded from this
-- rule however. These primitives are:
--
--   1. Clash.Signal.Internal.mapSignal#  (fmap)
--   2. Clash.Signal.Internal.appSignal#  (<*>)
--   3. Clash.Signal.Internal.signal#     (pure)
--
-- Note that a small number of other primitives (de)constructing _Signal_s
-- don't have a blackbox implementation either. For this explanation we can
-- ignore them though as they are very experimental code or performance
-- optimizations.
--
-- 'mapSignal#' (1) and 'appSignal#' (2) are both implemented very similarly, so
-- we'll only go over the first. Given the following:
--
--    f  :: a -> b
--    xs :: Signal dom a
--
...
--
-- In other words, 'xs' will be cast from a 'Signal dom a' to 'a' and given to
-- 'f' as its first argument. The result of that expression, of type 'a', is
-- cast back to 'Signal dom a'. This seems rather silly; something of type
-- 'Signal dom a' and 'a' have very different representations! However, in
-- Clash's context this is perfectly safe to do: all functions except (1), (2),
-- and (3) (de)constructing signals must have blackbox implementations. Hence,
-- Clash will never see a Signal constructor. This in turn means that the
-- difference in representation does not matter, making it safe to do the
-- coercion.
--
-- 'appSignal#' is implemented similarly to 'mapSignal#'. The implementation of
-- 'signal#' (pure) a bit different though, but allows us to do constant
-- elimination on signals producing a constant value. In pseudocode:
--
--    pure (x :: a)
--
-- is rewritten to:
...
--    $ x
--
-- `eliminateCastCast` can eliminate the back-to-back coercions, which in turn
  ...
  where
    ...
    term e
      ...
      where
        ...
        go "Clash.XException.xToErrorCtx" _ args
