...
import Clash.Core.Term (PrimInfo(..), IsMultiPrim(..), WorkInfo(..))
...
instance ToAst UFloat where
  toValue = const . pure . VLiteral . FloatLiteral . toRational . boxFloat
...
instance ToAst UDouble where
  toValue = const . pure . VLiteral . DoubleLiteral . toRational . boxDouble
