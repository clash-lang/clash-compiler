...
-}
module Clash.GHC.NetlistTypes (ghcTypeToHWType) where
...
import Clash.Core.Util (substArgTys, tyNatSize)
...
ghcTypeToHWType iw = go
  where
    ...
    go reprs m ty@(tyView -> TyConApp tc args) =
