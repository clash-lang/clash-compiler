...
-}
module Clash.GHC.GenerateBindings (generateBindings)
where
...
generateBindings opts startAction primDirs importDirs dbs hdl modName dflagsM = do
  ( bindings
    ...
    loadModules
      startAction
      (toGhcOverridingBool (opt_color opts))
      hdl
      modName
      dflagsM
      importDirs
  ...
  primMapR <-
    generatePrimMap unresolvedPrims primGuards (concat [pFP, primDirs, importDirs])
            ...
            case tms of
              ...
              _ ->
                let vsL = map (setIdScope LocalId . bindingId) tms
                    ...
                    tms1 =
                      zipWith
                        ( \b (i, _) -> (bindingId b, b{bindingTerm = Letrec lbs (Var i), bindingRecursive = False})
                        )
                        tms
                        lbs
                 in return tms1
