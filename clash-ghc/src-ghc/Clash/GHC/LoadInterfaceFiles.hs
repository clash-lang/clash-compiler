...
addBndrM hdl bndr exprM =
  case exprM of
    ...
    Just expr -> do
      ...
      modify $ \lb@LoadedBinders {..} ->
        lb
          { lbBinders = Map.insert bndr expr lbBinders,
...
loadIface env foundMod = do
  ifaceFailM <-
    LoadIface.findAndReadIface
      env
      (Outputable.text "loadIface")
      (fst (Module.getModuleInstantiation foundMod))
      foundMod
      UnitTypes.NotBoot
  case ifaceFailM of
      ...
      case target of
        ...
        Annotations.NamedTarget targetName0 ->
          let targetName1 = Text.unpack (qualifiedNameString' targetName0)
              ...
              primName = Text.unpack (name primOrErr)
           in if primName /= targetName1
                  ...
                  _ ->
                    error $
                      "Multiple primitive definitions found in "
                        ++ "InlinePrimitive annotation for "
                        ++ targetName1
                        ++ ". "
