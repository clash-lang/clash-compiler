...
maxBoundU = U $ (2 ^ natVal (SNat :: SNat n)) - 1
...
fromIntegerU_inlineable i = U $ i `mod` (2 ^ natVal (SNat :: SNat n))
...
finiteBitSizeU _ = fromInteger $ natVal (SNat :: SNat n)

instance forall n . KnownNat n => Lift (Unsigned n) where
  lift (U i) = sigE [| fromIntegerU i |] (decUnsigned $ natVal (SNat :: (SNat n)))
