...
fpAdder pipe (cell1, cell2) = (pipe', out)
 where
  newHead
    | valid cell1 = Just (value cell1 + value cell2, discr cell1)
    | otherwise = Nothing
  pipe' = newHead +>> pipe
  out = last pipe
...
resBuffer (Res{..}) (newDiscr, newDiscrVal, index, pipeCell, newCell) =
  ( Res
      { cellMem = cellMem'
      , indexMem = indexMem'
      }
 ...
 where
  ...
  cleanMem
    | newDiscr = replace newDiscrVal Nothing cellMem
    | otherwise = cellMem
  ...
  indexMem'
    | newDiscr = replace newDiscrVal index indexMem
    | otherwise = indexMem
  -- Value fed back into circuit
  resMemOut
    | valid pipeCell = cellMem !! (discr pipeCell)
    | otherwise = Nothing
  -- Value purged from the circuit
  redOut
    | valid (cellMem !! newDiscrVal) =
        Just (value (cellMem !! newDiscrVal), indexMem !! newDiscrVal)
    | otherwise = Nothing
...
reducer ::
  ...
  (Signal System DataInt, Signal System ArrayIndex) -> Signal System OutputSignal
