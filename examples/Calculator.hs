...
testBench = done
 where
  testInput =
    stimuliGenerator clk rst $ (listToVecTH [Imm 1 :: OPC Word, Push, Imm 2, Push, Pop, Pop, Pop, ADD])
  expectedOutput =
    outputVerifier' clk rst $
      (listToVecTH [Just 1 :: Maybe Word, Nothing, Just 2, Nothing, Nothing, Nothing, Nothing, Just 3])
