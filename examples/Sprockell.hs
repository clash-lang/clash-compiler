...
import Clash.Prelude hiding (And, Word)
...
data OpCode
  ...
  | Id
  | Incr
  | Decr -- no corresponding functions in prog.language
  | Neg
...
data Assembly
  = Compute OpCode RegBankAddr RegBankAddr RegBankAddr -- Compute opCode r0 r1 r2: go to "alu",
...
decode (pc, sp) instr = case instr of
  Compute c i0 i1 i2 ->
    nullcode{ldCode = LdAlu, opCode = c, fromreg0 = i0, fromreg1 = i1, toreg = i2}
  ...
  Pop r ->
    nullcode{ldCode = LdAddr, fromaddr = sp, toreg = r, spCode = Down}
  EndProg -> nullcode
  Debug _ -> nullcode
